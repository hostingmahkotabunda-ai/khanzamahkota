...
public final class RMResumeMedisRalanV2 extends JDialog {
    ...
    public RMResumeMedisRalanV2(Frame parent, boolean modal) {
        super(parent, modal);
        setTitle("Resume Rawat Jalan");
    ...
    private void ensureTable() {
        String sql =
            "create table if not exists resume_medis_ralan_v2 (" +
    ...
    private void ensureAksesResumeRalanV2() {
        try {
            if (Sequel.cariInteger("select count(*) from information_schema.columns where table_schema=database() and table_name='user' and column_name='data_resume_pasien'") == 0) {
                Sequel.queryu2("alter table user add column data_resume_pasien enum('true','false') default 'false'");
            }
        } catch (Exception e) {
            System.out.println("Notif akses Resume rawat jalan : " + e);
        }
    }

    private void ensureKolomTanggalKontrol() {
        try {
            if (Sequel.cariInteger("select count(*) from information_schema.columns where table_schema=database() and table_name='resume_medis_ralan_v2' and column_name='tanggal_kontrol'") == 0) {
                Sequel.queryu2("alter table resume_medis_ralan_v2 add column tanggal_kontrol varchar(255) null after instruksi_tindak_lanjut");
            }
        } catch (Exception e) {
    ...
    private void muatDaftarRuang() {
        ...
        try {
            stmt = koneksi.prepareStatement("select nm_poli from poliklinik where status='1' order by nm_poli");
            hasil = stmt.executeQuery();
            while (hasil.next()) {
                model.addElement(hasil.getString("nm_poli"));
            }
        } catch (Exception e) {
            System.out.println("Notif daftar poliklinik : " + e);
        } finally {
            try {
                if (hasil != null) {
                    hasil.close();
    ...
    public void tampilData() {
        ...
        try {
            String sql =
                ...
                "from resume_medis_ralan_v2 where tanggal_resume between ? and ? " +
    ...
    public void isCek() {
        boolean boleh = akses.getdata_resume_pasien();
    ...
    private void isRawat() {
        ...
        try {
            ps = koneksi.prepareStatement(
                ...
                "reg_periksa.kd_pj,penjab.png_jawab,poliklinik.nm_poli,reg_periksa.kd_dokter,dokter.nm_dokter from reg_periksa " +
            ...
            if (rs.next()) {
                ...
                setComboText(TRuang, nvl(rs.getString("nm_poli")));
                ...
                TTglKeluar.setText(nvl(rs.getString("tgl_registrasi")));
                TJamKeluar.setText(nvl(rs.getString("jam_reg")));
                if (ambil(TKodeDokter).isEmpty()) {
                    TKodeDokter.setText(nvl(rs.getString("kd_dokter")));
                    TNamaDokter.setText(nvl(rs.getString("nm_dokter")));
                }
            }
        } catch (Exception e) {
            System.out.println("Notif : " + e);
    ...
    private void muatDataJikaAda() {
        ...
        ResultSet hasil = null;
        try {
            stmt = koneksi.prepareStatement("select * from resume_medis_ralan_v2 where no_rawat=?");
    ...
    private boolean validasiInput() {
        if (ambil(TNoRw).isEmpty() || ambil(TNoRM).isEmpty() || ambil(TPasien).isEmpty()) {
            Valid.textKosong(TNoRw, "Pasien Rawat Jalan");
            return false;
        }
    ...
    private void simpanBaru() {
        ...
        try {
            stmt = koneksi.prepareStatement(
                "insert into resume_medis_ralan_v2 (" +
            ...
            stmt.executeUpdate();
            tampilData();
            widget.Toast.sukses(this, "Resume Rawat Jalan tersimpan.");
        } catch (Exception e) {
    ...
    private void updateData() {
        ...
        try {
            stmt = koneksi.prepareStatement(
                "update resume_medis_ralan_v2 set " +
            ...
            stmt.executeUpdate();
            tampilData();
            widget.Toast.sukses(this, "Resume Rawat Jalan diperbarui.");
        } catch (Exception e) {
    ...
    private void hapusData() {
        ...
        Sequel.meghapus("resume_medis_ralan_v2", "no_rawat", noRawat);
        tampilData();
        bersihkanForm();
        widget.Toast.sukses(this, "Resume Rawat Jalan dihapus.");
    }

    private boolean dataSudahAda() {
        return Sequel.cariInteger("select count(*) from resume_medis_ralan_v2 where no_rawat='" + ambil(TNoRw) + "'") > 0;
    }
    ...
    private void previewJasper() {
        ...
        try {
            siapkanReport("rptResumeMedisRalanV2");
            Valid.MyReportqry(
                "rptResumeMedisRalanV2.jasper",
                "report",
                "::[ Laporan Resume Rawat Jalan ]::",
                buatQueryCetak(),
                buatParameterCetak()
            );
        } catch (Exception e) {
    ...
    private static List<Map<String, Object>> buatBarisTabel(Connection konn, String noRawat) {
        ...
        String sql =
            ...
            "from resume_medis_ralan_v2 where no_rawat=?";
        try (PreparedStatement st = konn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = st.executeQuery()) {
                if (rs.next()) {
                    ...
                    String valKeluar = "1. " + rs.getString("diagnosa_keluar") + "\n2. " + rs.getString("diagnosa_sekunder1") + "\n3. " + rs.getString("diagnosa_sekunder2");
                    String kodeKeluar = kodeAtauStrip(rs.getString("icd10_keluar")) + "\n" + kodeAtauStrip(rs.getString("icd10_sekunder1")) + "\n" + kodeAtauStrip(rs.getString("icd10_sekunder2"));
                    tambahBaris(baris, "stetoskop", "Diagnosa Keluar", valKeluar, kodeKeluar);
                    ...
                    tambahBaris(baris, "obat", "Plan / Terapi / Obat", rs.getString("terapi_obat"), null);
                    tambahBaris(baris, "klip", "Instruksi Untuk Tindak Lanjut", rs.getString("instruksi_tindak_lanjut"), null);
                }
            }
        } catch (Exception e) {
            System.out.println("Notif baris tabel resume ralan V2 : " + e);
        }
        return baris;
    }
    ...
    private String buatQueryCetak() {
        return "select " +
            ...
            "ifnull((select d2.nm_dokter from dokter d2 where d2.kd_dokter=m.kd_dokter),ifnull(m.nm_dokter,'')) as dpjp," +
            "'tidak' as rawat_tim_dokter," +
            "concat('1. dr. ',ifnull(m.nm_dokter,'')) as tim_dokter1," +
            "'' as tim_dokter2," +
            "'' as tim_dokter3 " +
            "from resume_medis_ralan_v2 m " +
    ...
    /**
     * Cetak preview Jasper resume rawat jalan untuk no_rawat tertentu tanpa
    ...
    public static void cetak(String noRawat) {
        ...
        try {
            File jrxml = new File("./report/rptResumeMedisRalanV2.jrxml");
            File jasper = new File("./report/rptResumeMedisRalanV2.jasper");
            if (!jrxml.exists()) {
                throw new Exception("File report rptResumeMedisRalanV2 tidak ditemukan.");
            }
            if (!jasper.exists() || jrxml.lastModified() > jasper.lastModified()) {
                JasperCompileManager.compileReportToFile(jrxml.getPath(), jasper.getPath());
            }
            ...
            JOptionPane.showMessageDialog(null, "Gagal cetak resume rawat jalan : " + e.getMessage());
        }
    }
    ...
    private static Map<String, Object> buatParameterCetakStatis(String noRawat) {
        ...
        try (PreparedStatement st = konn.prepareStatement(
            ...
            System.out.println("Notif buatParameterCetakStatis (ralan) : " + e);
        }
    ...
    /** Isi field target di form (Keluhan Utama/Pemeriksaan Objek/Plan Terapi) dgn nilai SOAP yg
    ...
    private JLabel labelInfoSoap(String kolomTarget) {
        ...
        if ("keluhan".equals(kolomTarget)) {
            info = "Klik 2x pada satu baris utk langsung mengambilnya (atau centang beberapa lalu \"Ambil Yang Dicentang\" utk menggabungkan). Kolom Subjek akan dimasukkan ke Keluhan Utama.";
        } else if ("pemeriksaan".equals(kolomTarget)) {
            info = "Klik 2x pada satu baris utk langsung mengambilnya (atau centang beberapa lalu \"Ambil Yang Dicentang\" utk menggabungkan). Kolom Objek akan dimasukkan ke Pemeriksaan / Objek.";
        } else if ("rtl".equals(kolomTarget)) {
            info = "Klik 2x pada satu baris utk langsung mengambilnya (atau centang beberapa lalu \"Ambil Yang Dicentang\" utk menggabungkan). Kolom Plan akan dimasukkan ke Plan / Terapi / Obat.";
        }
    ...
    private void muatSoapDokter(DefaultTableModel modelSoap, String kolomWajib) {
        ...
        try {
            ...
            stmt = koneksi.prepareStatement(
                ...
                "ifnull(pemeriksaan_ralan." + kolomData + ",'') as data_soap," +
                "pemeriksaan_ralan.nip,pegawai.nama,ifnull(pegawai.jbtn,'') as jbtn " +
                ...
                "and ifnull(pemeriksaan_ralan." + kolomData + ",'')<>'' " +
    ...
    private JPanel buatPanelHero() {
        ...
        JLabel judul = new JLabel("Resume Rawat Jalan");
        judul.setFont(FONT_TITLE);
        judul.setForeground(WARNA_TEXT);

        JLabel subjudul = new JLabel("<html>Versi modern untuk resume Rawat Jalan, dokter otomatis dari login aktif, dan validasi tanda tangan langsung dari canvas gambar.</html>");
        ...
        badgePanel.add(buatBadge("Ralan Workflow", WARNA_ACCENT_SOFT, WARNA_ACCENT));
