...
/**
 * Early Warning Scoring System (Bayi), RM 9.1 REV001-03/23. Layout & wording mengikuti PERSIS
 * kertas asli yg dikirim user (wording TIDAK dikoreksi meski ada typo di kertas aslinya, spt
 * "Intraksi"/"Latergi"/"sianonis"/"Tanpak"/"derik" -- lihat OPSI_KATEGORI). Kertas aslinya cuma
 * sediakan 6 slot waktu penilaian per lembar (pola sama dgn RMAsesmenRisikoJatuhBayiAnak yg cuma
 * 5 slot per lembar), jadi 1 baris tabel = 1 pasien (no_rawat), kolom lebar a1_..a6_.. per slot.
 */
public final class RMEWSBayi extends JDialog {
    ...
    // Wording & skor VERBATIM kertas asli (RM 9.1) -- termasuk typo "Intraksi","Latergi","sianonis","Tanpak","derik".
    ...
    /** Baris grid: 1 baris per opsi (12 baris tetap = 3 kategori x 4 opsi, tidak ada pemangkasan spt Risiko Jatuh). */
    ...
    public RMEWSBayi(Frame parent, boolean modal) {
        super(parent, modal);
        setTitle("::[ Early Warning Scoring System (Bayi) - RM 9.1 ]::");
    ...
    private void initComponents() {
        ...
        JLabel judulUtama = new JLabel("Early Warning Scoring System (Bayi)");
        ...
        // ----- Baris tanggal/jam Penilaian 1-6 -----
    ...
    /** Tabel referensi Usia vs Nadi/Nafas saat istirahat -- statis, sama persis kertas asli, bukan data pasien. */
    ...
    /** Legenda "LEVEL SKOR & RESPON" -- statis, sama persis kertas asli. */
     ...
     *  tidak tumpang tindih spt sebelumnya (dulu tinggi baris tetap 34px utk semua baris, padahal
     *  ada baris dgn teks panjang yg butuh 2-3 baris tampilan). */
    ...
    /** Dipanggil dari tab EWS (dropdown "EWS Bayi") di DlgRawatInap/DlgRawatJalan. */
    ...
    private void tarikIdentitasPasien(String norawat) {
        try (PreparedStatement ps = koneksi.prepareStatement(
            ...
            System.out.println("Notif tarik identitas EWS bayi : " + e);
        }
    }

    private void muat(String norawat) {
        try (PreparedStatement ps = koneksi.prepareStatement("select * from ews_bayi where no_rawat=?")) {
            ps.setString(1, norawat);
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
            ...
            sedangMemuat = false;
            System.out.println("Notif muat EWS bayi : " + e);
        }
    }
    ...
    private void simpan() {
        ...
        String sql = "insert into ews_bayi (" + kolom + ") values (" + placeholder + ") "
        ...
        try (PreparedStatement ps = koneksi.prepareStatement(sql)) {
            ...
            ps.executeUpdate();
            JOptionPane.showMessageDialog(this, "EWS Bayi tersimpan.");
            muat(noRawat);
        } catch (Exception e) {
    ...
    private void ensureTable() {
        StringBuilder sql = new StringBuilder("create table if not exists ews_bayi (no_rawat varchar(17) not null primary key,");
        for (int s = 1; s <= N_SLOT; s++) {
    ...
    /** Cetak langsung dari no_rawat tanpa membuka dialog (dipakai dari klik-kanan/kartu blok di
     *  layar Riwayat Perawatan) -- pola sama persis RMAsesmenUlangNyeri.cetak(String). */
    public static void cetak(String noRawat) {
        if (noRawat == null || noRawat.trim().isEmpty()) { return; }
        RMEWSBayi f = new RMEWSBayi(null, false);
        f.isCek();
    ...
    /** Cetak Jasper (rptEWSBayi.jasper) -- grid 1 baris = 1 kriteria (bukan 1 baris = 1 pasien) supaya
     *  JasperReports otomatis pindah halaman kalau tabel kepanjangan. SQL dibangun sbg UNION ALL per
     *  baris GRID_ROWS, kolom cN='Y' kalau opsi baris ini yg tersimpan di slot ke-N. */
    private void cetak() {
        if (noRawat.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Pilih pasien terlebih dahulu.");
            return;
        }
        if (Sequel.cariInteger("select count(*) from ews_bayi where no_rawat=?", noRawat) == 0) {
        ...
        try {
            ...
            for (int s = 1; s <= N_SLOT; s++) {
                param.put("tgl" + s, Sequel.cariIsi("select ifnull(date_format(a" + s + "_tanggal,'%d-%m-%Y %H:%i'),'-') from ews_bayi where no_rawat='" + noRawatEsc + "'"));
                param.put("total" + s, Sequel.cariIsi("select ifnull(a" + s + "_total_skor,'-') from ews_bayi where no_rawat='" + noRawatEsc + "'"));
                param.put("level" + s, printSafe(Sequel.cariIsi("select ifnull(a" + s + "_level,'-') from ews_bayi where no_rawat='" + noRawatEsc + "'")));
            }
            ...
            for (int k = 0; k < KATEGORI_KEY.length; k++) {
                for (int o = 0; o < OPSI_KATEGORI[k].length; o++) {
                    ...
                    sql.append(" from ews_bayi where no_rawat='").append(noRawatEsc).append("'");
                    urutan++;
                }
            }
            sql.append(" order by urutan");
            Valid.MyReportqry("rptEWSBayi.jasper", "report", "::[ Early Warning Scoring System (Bayi) ]::", sql.toString(), param);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Gagal mencetak.\n" + e.getMessage());
        }
    }

    /** Font PDF standar (Cp1252, sama spt semua .jrxml lain di proyek ini) tidak punya glyph
     *  U+2265/U+2264 (≥/≤) -- diganti ASCII ">="/"<=" KHUSUS utk cetakan (layar & DB tetap apa adanya). */
    ...
    /** Kolom Kategori: label kategori (Keadaan Umum/Kardiovaskular/Respirasi) cuma tampil di baris
     *  tengah kelompoknya (meniru sel gabungan di kertas), sama pola dgn FaktorRenderer template
     *  Risiko Jatuh. */
    ...
    /** Kolom Kriteria: teks opsi (nomor+deskripsi) -- dibungkus HTML supaya word-wrap rapi
     *  mengikuti lebar kolom yg sebenarnya (dipakai jg oleh aturTinggiBarisOtomatis()). */
    ...
    /** Kolom Skor + kolom checkbox slot -- diwarnai sesuai warna opsi kertas asli (KEADAAN_UMUM/KARDIOVASKULAR/RESPIRASI). */
