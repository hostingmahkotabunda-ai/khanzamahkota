...
import rekammedis.RMResumeMedisRalanV2;
import permintaan.DlgPermintaanLaboratorium;
import permintaan.DlgPermintaanPelayananInformasiObat;
import permintaan.DlgPermintaanRadiologi;
import rekammedis.DlgTemplateSOAPExcel;
...
public final class DlgRawatJalan extends javax.swing.JDialog {
    ...
    private DefaultTableModel tabModeSoapDokter;
    private DefaultTableModel tabModeSoapPerawat;
    private DlgPeresepanDokter resepTerintegrasiRalan;
    private String konteksResepTerintegrasi="";
    private boolean modeEmbeddedPemeriksaan=false;
    private DefaultTableModel tabModeRiwayatPasienRalan;
    private javax.swing.JEditorPane htmlDetailRiwayatRalan;
    ...
    private javax.swing.JComboBox<String> cmbDPRJK, cmbDPRStatusNikah, cmbDPRAgama, cmbDPRPendidikan, cmbDPRKeluarga;
    private javax.swing.JButton btnDPREdit, btnDPRSimpan, btnDPRMuat;
    private boolean modeEditDataPasienRalan=false;
    


    private void sesuaikanTabRalanDenganRanap() {
        try { TabRawat.remove(internalFrame2); } catch (Exception e) {}
        try { TabRawat.remove(internalFrame3); } catch (Exception e) {}
        try { TabRawat.remove(internalFrame6); } catch (Exception e) {}
        try { TabRawat.remove(internalFrame7); } catch (Exception e) {}
        try { TabRawat.remove(internalFrame8); } catch (Exception e) {} // Catatan Dokter
        try { TabRawat.setSelectedComponent(internalFrame5); } catch (Exception e) {}
    }

    private int idxTabRawatRalan() {
        java.awt.Component c=TabRawat.getSelectedComponent();
        if(c==internalFrame2) return 0;
        if(c==internalFrame3) return 1;
        if(c==internalFrame4) return 2;
        if(c==internalFrame5) return 3;
        if(c==internalFrame6) return 4;
        if(c==internalFrame7) return 5;
        if(c==panelDiagnosa1) return 6;
        if(c==internalFrame8) return 7;
        if(c==PanelResepTerintegrasi) return 8;
    ...
    /**
     * Tab "EWS" (Early Warning Scoring System) -- pola sama persis dgn tab "Penilaian Awal"
     ...
     * menu supaya strukturnya sudah siap, tinggal isi bukaEWSAnakRalan()/bukaEWSDewasaRalan() nanti.
     */
    private void pasangTabEWSRalan() {
        ...
        menuEWS.add(itemMenuPenilaianAwalRalan("EWS Bayi (RM 9.1)", e -> bukaEWSBayiRalan()));
        menuEWS.add(itemMenuPenilaianAwalRalan("EWS Anak", e -> bukaEWSAnakRalan()));
        menuEWS.add(itemMenuPenilaianAwalRalan("EWS Dewasa", e -> bukaEWSDewasaRalan()));
    ...
    /** Buka form Early Warning Scoring System (Bayi), RM 9.1, untuk pasien ralan yang aktif. */
    private void bukaEWSBayiRalan() {
        if (TNoRw.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Pilih pasien terlebih dahulu.");
            return;
        }
    ...
    /** Buka form Early Warning Scoring System (Anak), RM 9.1, untuk pasien ralan yang aktif. */
    private void bukaEWSAnakRalan() {
        if (TNoRw.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Pilih pasien terlebih dahulu.");
            return;
        }
    ...
    /** Buka form Early Warning Scoring System (Dewasa), RM 9.1, untuk pasien ralan yang aktif. */
    private void bukaEWSDewasaRalan() {
        if (TNoRw.getText().trim().equals("")) {
            JOptionPane.showMessageDialog(this, "Pilih pasien terlebih dahulu.");
            return;
        }
    ...
    private void pasangTabDataPasienRalan() {
        final java.awt.Color bg=new java.awt.Color(238,243,238);
        panelDataPasienRalan=new javax.swing.JPanel(new java.awt.BorderLayout(8,8));
        panelDataPasienRalan.setName("panelDataPasienRalan");
        panelDataPasienRalan.setBackground(bg);
        panelDataPasienRalan.setBorder(javax.swing.BorderFactory.createEmptyBorder(8,8,8,8));
        pastikanTabelCatatanPasienRalan();

        // Kolom 0-7 tampil, kolom 8-11 (SOAP) disembunyikan untuk panel detail
        tabModeRiwayatPasienRalan=new DefaultTableModel(null,new Object[]{
            "No. Kunjungan","Tanggal Kunjungan","Poli","Dokter","Status","Diagnosa","Cara Bayar","Petugas Input",
            "Subjek","Objek","Asesmen","Plan"
        }){
            @Override public boolean isCellEditable(int rowIndex, int colIndex){
                return false;
            }
        };
        tbRiwayatPasienRalan=new widget.Table();
        tbRiwayatPasienRalan.setModel(tabModeRiwayatPasienRalan);
        tbRiwayatPasienRalan.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        tbRiwayatPasienRalan.setRowHeight(26);
        tbRiwayatPasienRalan.setDefaultRenderer(Object.class, new WarnaTable());
        int[] lebar={120,130,100,150,80,160,95,100};
        for(int kol=0;kol<lebar.length;kol++){
            tbRiwayatPasienRalan.getColumnModel().getColumn(kol).setPreferredWidth(lebar[kol]);
        }
        // Renderer badge untuk kolom Status
        final javax.swing.table.DefaultTableCellRenderer badge=new javax.swing.table.DefaultTableCellRenderer(){
            @Override public java.awt.Component getTableCellRendererComponent(JTable t,Object v,boolean sel,boolean foc,int r,int c){
                ...
                l.setOpaque(true);
                l.setBorder(javax.swing.BorderFactory.createEmptyBorder(2,6,2,6));
                String s=v==null?"":v.toString().trim();
                if(!s.isEmpty()){
                    l.setBackground(new java.awt.Color(223,245,231));
                    l.setForeground(new java.awt.Color(22,120,60));
                }else{
        ...
        tbRiwayatPasienRalan.getColumnModel().getColumn(4).setCellRenderer(badge);
        // Sembunyikan kolom SOAP (8..11) dari tampilan, tetap ada di model untuk detail
        for(int idx=11;idx>=8;idx--){
            tbRiwayatPasienRalan.getColumnModel().removeColumn(tbRiwayatPasienRalan.getColumnModel().getColumn(idx));
        }
        javax.swing.table.JTableHeader hdr=tbRiwayatPasienRalan.getTableHeader();
        hdr.setReorderingAllowed(false);
        hdr.setFont(new Font("Tahoma", Font.BOLD, 11));

        htmlDetailRiwayatRalan=new javax.swing.JEditorPane();
        htmlDetailRiwayatRalan.setContentType("text/html");
        htmlDetailRiwayatRalan.setEditable(false);
        htmlDetailRiwayatRalan.putClientProperty(javax.swing.JEditorPane.HONOR_DISPLAY_PROPERTIES, Boolean.TRUE);
        htmlDetailRiwayatRalan.setFont(new Font("Tahoma", Font.PLAIN, 12));
        htmlDetailRiwayatRalan.setBackground(java.awt.Color.WHITE);
        htmlDetailRiwayatRalan.setText("<html><body style='font-family:Tahoma;font-size:11px;color:#888;margin:8px;'>Klik salah satu baris riwayat untuk melihat detail lengkap.</body></html>");

        tbRiwayatPasienRalan.addMouseListener(new java.awt.event.MouseAdapter(){
            @Override public void mouseClicked(java.awt.event.MouseEvent e){
                int r=tbRiwayatPasienRalan.getSelectedRow();
                if(r!=-1){ tampilDetailRiwayatRalan(tbRiwayatPasienRalan.convertRowIndexToModel(r)); }
            }
        });

        widget.ScrollPane scrollDataPasien=new widget.ScrollPane();
        scrollDataPasien.setViewportView(buatFormDataPasienRalan());
        scrollDataPasien.setBorder(null);
        widget.ScrollPane scrollRiwayatPasien=new widget.ScrollPane();
        scrollRiwayatPasien.setViewportView(tbRiwayatPasienRalan);
        scrollRiwayatPasien.setBorder(null);
        widget.ScrollPane scrollDetailRiwayat=new widget.ScrollPane();
        scrollDetailRiwayat.setViewportView(htmlDetailRiwayatRalan);
        scrollDetailRiwayat.setBorder(null);

        javax.swing.JPanel cardData=kartuDataPasien("Data Pasien", scrollDataPasien);
        javax.swing.JPanel cardRiwayat=kartuDataPasien("Riwayat Kunjungan", scrollRiwayatPasien);
        javax.swing.JPanel cardDetail=kartuDataPasien("Detail Riwayat Berobat", scrollDetailRiwayat);

        // Kanan: riwayat (atas) + detail (bawah)
        javax.swing.JSplitPane splitKanan=new javax.swing.JSplitPane(
                javax.swing.JSplitPane.VERTICAL_SPLIT, cardRiwayat, cardDetail);
        splitKanan.setResizeWeight(0.55);
        splitKanan.setBorder(null);
        splitKanan.setOpaque(false);
        splitKanan.setBackground(bg);

        // Utama: data diri (kiri) | riwayat+detail (kanan)
        javax.swing.JSplitPane split=new javax.swing.JSplitPane(
                javax.swing.JSplitPane.HORIZONTAL_SPLIT, cardData, splitKanan);
        split.setResizeWeight(0.30);
        split.setBorder(null);
        split.setOpaque(false);
        split.setBackground(bg);
        panelDataPasienRalan.add(split, java.awt.BorderLayout.CENTER);
        TabRawat.addTab("Data Pasien", panelDataPasienRalan);
    }

    private javax.swing.JPanel buatFormDataPasienRalan() {
        ...
        txtDPRNoRM=fieldDataPasien(false);
        txtDPRNama=fieldDataPasien(true);
        txtDPRNIK=fieldDataPasien(true);
        txtDPRTempatLahir=fieldDataPasien(true);
        txtDPRTglLahir=fieldDataPasien(true);
        txtDPRTelepon=fieldDataPasien(true);
        txtDPRPekerjaan=fieldDataPasien(true);
        txtDPRNamaKeluarga=fieldDataPasien(true);
        txtDPRPekerjaanPJ=fieldDataPasien(true);
        txtDPRAlamat=areaDataPasien(3);
        txtDPRCatatan=areaDataPasien(5);
        cmbDPRJK=new javax.swing.JComboBox<>(new String[]{"L","P"});
        cmbDPRStatusNikah=new javax.swing.JComboBox<>(new String[]{"BELUM MENIKAH","MENIKAH","JANDA","DUDHA","JOMBLO","-"});
        cmbDPRAgama=new javax.swing.JComboBox<>(new String[]{"ISLAM","KRISTEN","KATOLIK","HINDU","BUDHA","KONG HU CHU","-"});
        cmbDPRPendidikan=new javax.swing.JComboBox<>(new String[]{"TS","TK","SD","SMP","SMA","SLTA/SEDERAJAT","D1","D2","D3","D4","S1","S2","S3","-"});
        cmbDPRKeluarga=new javax.swing.JComboBox<>(new String[]{"AYAH","IBU","ISTRI","SUAMI","SAUDARA","ANAK","DIRI SENDIRI","LAIN-LAIN","-"});

        int row=0;
        tambahBarisFormPasien(form,gbc,row++,"No. RM",txtDPRNoRM);
        tambahBarisFormPasien(form,gbc,row++,"Nama Pasien",txtDPRNama);
        tambahBarisFormPasien(form,gbc,row++,"NIK",txtDPRNIK);
        tambahBarisFormPasien(form,gbc,row++,"Jenis Kelamin",cmbDPRJK);
        tambahBarisFormPasien(form,gbc,row++,"Tempat Lahir",txtDPRTempatLahir);
        tambahBarisFormPasien(form,gbc,row++,"Tgl Lahir",txtDPRTglLahir);
        tambahBarisFormPasien(form,gbc,row++,"Alamat",new javax.swing.JScrollPane(txtDPRAlamat));
        tambahBarisFormPasien(form,gbc,row++,"No. Telepon",txtDPRTelepon);
        tambahBarisFormPasien(form,gbc,row++,"Pendidikan",cmbDPRPendidikan);
        tambahBarisFormPasien(form,gbc,row++,"Pekerjaan",txtDPRPekerjaan);
        tambahBarisFormPasien(form,gbc,row++,"Status Nikah",cmbDPRStatusNikah);
        tambahBarisFormPasien(form,gbc,row++,"Agama",cmbDPRAgama);
        tambahBarisFormPasien(form,gbc,row++,"Keluarga/PJ",cmbDPRKeluarga);
        tambahBarisFormPasien(form,gbc,row++,"Nama Keluarga",txtDPRNamaKeluarga);
        tambahBarisFormPasien(form,gbc,row++,"Pekerjaan PJ",txtDPRPekerjaanPJ);
        tambahBarisFormPasien(form,gbc,row++,"Catatan Pasien",new javax.swing.JScrollPane(txtDPRCatatan));

        javax.swing.JPanel tombol=new javax.swing.JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.RIGHT,6,0));
        tombol.setBackground(java.awt.Color.WHITE);
        btnDPREdit=new javax.swing.JButton("Edit");
        btnDPRMuat=new javax.swing.JButton("Muat Ulang");
        btnDPRSimpan=new javax.swing.JButton("Simpan");
        btnDPREdit.setFont(new Font("Tahoma", Font.BOLD, 11));
        btnDPRMuat.setFont(new Font("Tahoma", Font.PLAIN, 11));
        btnDPRSimpan.setFont(new Font("Tahoma", Font.BOLD, 11));
        btnDPREdit.addActionListener(new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
                setModeEditDataPasienRalan(true);
                txtDPRNama.requestFocus();
            }
        });
        btnDPRMuat.addActionListener(new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
                tampilIdentitasPasienRalan();
            }
        });
        btnDPRSimpan.addActionListener(new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
                simpanDataPasienRalan();
            }
        });
        tombol.add(btnDPREdit);
        tombol.add(btnDPRMuat);
        tombol.add(btnDPRSimpan);

        panel.add(form, java.awt.BorderLayout.CENTER);
        panel.add(tombol, java.awt.BorderLayout.SOUTH);
        setModeEditDataPasienRalan(false);
        return panel;
    }

    private javax.swing.JTextField fieldDataPasien(boolean editable) {
    ...
    private javax.swing.JTextArea areaDataPasien(int baris) {
        javax.swing.JTextArea area=new javax.swing.JTextArea(baris,20);
    ...
    private void tambahBarisFormPasien(javax.swing.JPanel form, java.awt.GridBagConstraints gbc, int row, String label, java.awt.Component input) {
        javax.swing.JLabel lbl=new javax.swing.JLabel(label);
        lbl.setFont(new Font("Tahoma", Font.PLAIN, 11));
        lbl.setForeground(new java.awt.Color(72,86,75));
        gbc.gridx=0;
        gbc.gridy=row;
        gbc.weightx=0;
        gbc.fill=java.awt.GridBagConstraints.HORIZONTAL;
    ...
    private void pastikanTabelCatatanPasienRalan() {
        Sequel.queryu("create table if not exists catatan_pasien_ralan("+
                "no_rkm_medis varchar(15) not null,"+
                "catatan text,"+
                "tgl_update datetime not null,"+
                "petugas varchar(20) not null default '',"+
                "primary key(no_rkm_medis)) engine=InnoDB default charset=latin1");
    }

    private void setModeEditDataPasienRalan(boolean edit) {
        modeEditDataPasienRalan=edit;
        if(txtDPRNama==null){ return; }
        txtDPRNama.setEditable(edit);
        txtDPRNIK.setEditable(edit);
        txtDPRTempatLahir.setEditable(edit);
        txtDPRTglLahir.setEditable(edit);
        txtDPRAlamat.setEditable(edit);
        txtDPRTelepon.setEditable(edit);
        txtDPRPekerjaan.setEditable(edit);
        txtDPRNamaKeluarga.setEditable(edit);
        txtDPRPekerjaanPJ.setEditable(edit);
        txtDPRCatatan.setEditable(edit);
        cmbDPRJK.setEnabled(edit);
        cmbDPRStatusNikah.setEnabled(edit);
        cmbDPRAgama.setEnabled(edit);
        cmbDPRPendidikan.setEnabled(edit);
        cmbDPRKeluarga.setEnabled(edit);
        if(btnDPREdit!=null){
            btnDPREdit.setEnabled(!edit);
            btnDPRSimpan.setEnabled(edit);
        }
    }

    /** Beri simbol bulatan berwarna berbeda pada tiap tab agar mudah dibedakan & menarik. */
    private void hiasTabRalan() {
        java.awt.Color[] warna = {
            new java.awt.Color(33,150,243),   // biru
            new java.awt.Color(0,150,136),    // teal
            new java.awt.Color(255,152,0),    // oranye
            new java.awt.Color(156,39,176),   // ungu
            new java.awt.Color(76,175,80),    // hijau
            new java.awt.Color(233,30,99),    // pink
            new java.awt.Color(121,85,72),    // coklat
            new java.awt.Color(96,125,139),   // abu biru
            new java.awt.Color(0,188,212),    // cyan
            new java.awt.Color(205,140,0)     // amber tua
        };
        for(int i=0;i<TabRawat.getTabCount();i++){
            String judul=TabRawat.getTitleAt(i);
            if(judul==null){ judul=""; }
            java.awt.Color c=warna[i%warna.length];
            String hex=String.format("#%02x%02x%02x",c.getRed(),c.getGreen(),c.getBlue());
            javax.swing.JLabel lab=new javax.swing.JLabel(
                "<html><font color='"+hex+"'>●</font>&nbsp;<font color='#33414d'>"+judul+"</font></html>");
            lab.setFont(new Font("Tahoma", Font.BOLD, 11));
            lab.setOpaque(false);
            lab.setBorder(javax.swing.BorderFactory.createEmptyBorder(1,2,1,4));
            TabRawat.setTabComponentAt(i, lab);
        }
    }

    /** Kartu putih dengan judul untuk tab Data Pasien (meniru layout kartu). */
    private javax.swing.JPanel kartuDataPasien(String judul, java.awt.Component isi) {
        javax.swing.JPanel card=new javax.swing.JPanel(new java.awt.BorderLayout(0,6));
        card.setBackground(java.awt.Color.WHITE);
        card.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                javax.swing.BorderFactory.createLineBorder(new java.awt.Color(223,232,224)),
                javax.swing.BorderFactory.createEmptyBorder(8,10,8,10)));
        javax.swing.JLabel head=new javax.swing.JLabel(judul);
        head.setFont(new Font("Tahoma", Font.BOLD, 13));
        head.setForeground(new java.awt.Color(34,90,52));
        card.add(head, java.awt.BorderLayout.NORTH);
        card.add(isi, java.awt.BorderLayout.CENTER);
        return card;
    }

    private void tampilDetailRiwayatRalan(int row) {
        if(row<0 || row>=tabModeRiwayatPasienRalan.getRowCount()){ return; }
        StringBuilder h=new StringBuilder();
        h.append("<html><head><style>")
                .append("body{font-family:Tahoma;font-size:11px;color:#202020;margin:6px;}")
                .append("table{width:100%;border-collapse:collapse;}")
                .append(".meta td{padding:3px 4px;vertical-align:top;width:50%;}")
                .append(".lab{color:#6b7a70;}")
                .append(".val{font-weight:bold;color:#22321f;}")
                .append(".soaplab{font-weight:bold;color:#2e7d57;margin:8px 0 2px 0;}")
                .append(".soapval{margin:0 0 6px 0;}")
                .append("hr{border:0;border-top:1px solid #eef3ee;margin:6px 0;}")
                .append("</style></head><body>")
                .append("<table class='meta'><tr><td>")
                .append("<div><span class='lab'>No. Kunjungan :</span> <span class='val'>").append(html(nilai(valRiwayat(row,0)))).append("</span></div>")
                .append("<div><span class='lab'>Tanggal :</span> <span class='val'>").append(html(nilai(valRiwayat(row,1)))).append("</span></div>")
                .append("<div><span class='lab'>Poli :</span> <span class='val'>").append(html(nilai(valRiwayat(row,2)))).append("</span></div>")
                .append("<div><span class='lab'>Dokter :</span> <span class='val'>").append(html(nilai(valRiwayat(row,3)))).append("</span></div>")
                .append("</td><td>")
                .append("<div><span class='lab'>Cara Bayar :</span> <span class='val'>").append(html(nilai(valRiwayat(row,6)))).append("</span></div>")
                .append("<div><span class='lab'>Petugas Input :</span> <span class='val'>").append(html(nilai(valRiwayat(row,7)))).append("</span></div>")
                .append("<div><span class='lab'>Status :</span> <span class='val'>").append(html(nilai(valRiwayat(row,4)))).append("</span></div>")
                .append("<div><span class='lab'>Diagnosa :</span> <span class='val'>").append(html(nilai(valRiwayat(row,5)))).append("</span></div>")
                .append("</td></tr></table><hr>")
                .append("<div class='soaplab'>Subyektif (S)</div><div class='soapval'>").append(html(nilai(valRiwayat(row,8)))).append("</div>")
    ...
    private void tampilDataPasienRalan() {
        tampilIdentitasPasienRalan();
        tampilRiwayatPasienRalan();
    }

    private void tampilIdentitasPasienRalan() {
        kosongkanFormDataPasienRalan();
        if(TNoRM.getText().trim().equals("")){
            setModeEditDataPasienRalan(false);
            return;
        }
        String pekerjaanPj=Sequel.cariInteger("select count(*) from information_schema.columns where table_schema=database() and table_name='pasien' and column_name='pekerjaanpj'")>0 ?
                "pasien.pekerjaanpj" : "''";
        try (PreparedStatement psPasien=koneksi.prepareStatement(
                "select pasien.no_rkm_medis,pasien.nm_pasien,pasien.no_ktp,pasien.jk,pasien.tmp_lahir,pasien.tgl_lahir,pasien.umur,"+
                "pasien.alamat,pasien.no_tlp,pasien.stts_nikah,pasien.agama,pasien.pnd,pasien.pekerjaan,pasien.keluarga,pasien.namakeluarga,"+
                pekerjaanPj+" as pekerjaanpj from pasien where pasien.no_rkm_medis=?")) {
            psPasien.setString(1,TNoRM.getText());
            try (ResultSet rsPasien=psPasien.executeQuery()) {
                if(rsPasien.next()){
                    txtDPRNoRM.setText(nilai(rsPasien.getString("no_rkm_medis")));
                    txtDPRNama.setText(nilai(rsPasien.getString("nm_pasien")));
                    txtDPRNIK.setText(nilai(rsPasien.getString("no_ktp")));
                    setComboDataPasien(cmbDPRJK,nilai(rsPasien.getString("jk")));
                    txtDPRTempatLahir.setText(nilai(rsPasien.getString("tmp_lahir")));
                    txtDPRTglLahir.setText(nilai(rsPasien.getString("tgl_lahir")));
                    txtDPRAlamat.setText(nilai(rsPasien.getString("alamat")));
                    txtDPRTelepon.setText(nilai(rsPasien.getString("no_tlp")));
                    setComboDataPasien(cmbDPRStatusNikah,nilai(rsPasien.getString("stts_nikah")));
                    setComboDataPasien(cmbDPRAgama,nilai(rsPasien.getString("agama")));
                    setComboDataPasien(cmbDPRPendidikan,nilai(rsPasien.getString("pnd")));
                    txtDPRPekerjaan.setText(nilai(rsPasien.getString("pekerjaan")));
                    setComboDataPasien(cmbDPRKeluarga,nilai(rsPasien.getString("keluarga")));
                    txtDPRNamaKeluarga.setText(nilai(rsPasien.getString("namakeluarga")));
                    txtDPRPekerjaanPJ.setText(nilai(rsPasien.getString("pekerjaanpj")));
                    txtDPRCatatan.setText(Sequel.cariIsi("select catatan from catatan_pasien_ralan where no_rkm_medis=?",TNoRM.getText()));
                    txtDPRCatatan.setCaretPosition(0);
                }
            }
        } catch (Exception e) {
            System.out.println("Notifikasi Data Pasien : "+e);
        }
        setModeEditDataPasienRalan(false);
    }

    private void kosongkanFormDataPasienRalan() {
        if(txtDPRNoRM==null){ return; }
        txtDPRNoRM.setText("");
        txtDPRNama.setText("");
        txtDPRNIK.setText("");
        txtDPRTempatLahir.setText("");
        txtDPRTglLahir.setText("");
        txtDPRAlamat.setText("");
        txtDPRTelepon.setText("");
        txtDPRPekerjaan.setText("");
        txtDPRNamaKeluarga.setText("");
        txtDPRPekerjaanPJ.setText("");
        txtDPRCatatan.setText("");
    }

    private void setComboDataPasien(javax.swing.JComboBox<String> combo, String nilai) {
        String value=nilai(nilai);
    ...
    private void simpanDataPasienRalan() {
        if(!modeEditDataPasienRalan){
            JOptionPane.showMessageDialog(null,"Klik tombol Edit terlebih dahulu sebelum mengubah data pasien.");
            return;
        }
        if(txtDPRNoRM==null || txtDPRNoRM.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Pilih pasien terlebih dahulu.");
            return;
        }
        if(txtDPRNama.getText().trim().equals("")){
            Valid.textKosong(txtDPRNama,"Nama pasien");
            return;
        }
        try {
            simpanIdentitasPasienRalan();
            simpanCatatanPasienRalan();
            TPasien.setText(txtDPRNama.getText().trim());
            JOptionPane.showMessageDialog(null,"Data pasien berhasil disimpan.");
            tampilDataPasienRalan();
            setModeEditDataPasienRalan(false);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null,"Gagal menyimpan data pasien : "+e.getMessage());
            System.out.println("Notifikasi Simpan Data Pasien : "+e);
        }
    }

    private void simpanIdentitasPasienRalan() throws Exception {
        boolean adaPekerjaanPj=Sequel.cariInteger("select count(*) from information_schema.columns where table_schema=database() and table_name='pasien' and column_name='pekerjaanpj'")>0;
        String sql="update pasien set nm_pasien=?,no_ktp=?,jk=?,tmp_lahir=?,tgl_lahir=?,alamat=?,no_tlp=?,"+
                "stts_nikah=?,agama=?,pnd=?,pekerjaan=?,keluarga=?,namakeluarga=?,umur=concat(timestampdiff(year,tgl_lahir,curdate()),' Th')"+
                (adaPekerjaanPj ? ",pekerjaanpj=?" : "")+" where no_rkm_medis=?";
        try (PreparedStatement psSimpan=koneksi.prepareStatement(sql)) {
            int idx=1;
            psSimpan.setString(idx++,txtDPRNama.getText().trim());
            psSimpan.setString(idx++,txtDPRNIK.getText().trim());
            psSimpan.setString(idx++,cmbDPRJK.getSelectedItem().toString());
            psSimpan.setString(idx++,txtDPRTempatLahir.getText().trim());
            psSimpan.setString(idx++,txtDPRTglLahir.getText().trim());
            psSimpan.setString(idx++,txtDPRAlamat.getText().trim());
            psSimpan.setString(idx++,txtDPRTelepon.getText().trim());
            psSimpan.setString(idx++,cmbDPRStatusNikah.getSelectedItem().toString());
            psSimpan.setString(idx++,cmbDPRAgama.getSelectedItem().toString());
            psSimpan.setString(idx++,cmbDPRPendidikan.getSelectedItem().toString());
            psSimpan.setString(idx++,txtDPRPekerjaan.getText().trim());
            psSimpan.setString(idx++,cmbDPRKeluarga.getSelectedItem().toString());
            psSimpan.setString(idx++,txtDPRNamaKeluarga.getText().trim());
            if(adaPekerjaanPj){
                psSimpan.setString(idx++,txtDPRPekerjaanPJ.getText().trim());
            }
            psSimpan.setString(idx,txtDPRNoRM.getText().trim());
            psSimpan.executeUpdate();
        }
    }

    private void simpanCatatanPasienRalan() throws Exception {
        try (PreparedStatement psSimpan=koneksi.prepareStatement(
                "insert into catatan_pasien_ralan(no_rkm_medis,catatan,tgl_update,petugas) values(?,?,now(),?) "+
                "on duplicate key update catatan=values(catatan),tgl_update=now(),petugas=values(petugas)")) {
            psSimpan.setString(1,txtDPRNoRM.getText().trim());
            psSimpan.setString(2,txtDPRCatatan.getText().trim());
            psSimpan.setString(3,akses.getkode());
            psSimpan.executeUpdate();
        }
    }

    private String barisDataPasien(String label1,String value1,String label2,String value2) {
        return "<tr><td class='label'>"+html(label1)+"</td><td>"+html(nilai(value1))+"</td>"+
                "<td class='label'>"+html(label2)+"</td><td>"+html(nilai(value2))+"</td></tr>";
    }

    private String barisAtas(String label,String valueHtml) {
        return "<tr class='row'><td width='40%'><span class='lbl'>"+html(label)+"</span></td><td>"+valueHtml+"</td></tr>";
    }

    private String valSpan(String value) {
        return "<span class='val'>"+html(nilai(value))+"</span>";
    }

    private String selGrid(String label,String value) {
        return "<td><span class='lbl'>"+html(label)+"</span><br><span class='val'>"+html(nilai(value))+"</span></td>";
    }

    private void tampilRiwayatPasienRalan() {
        Valid.tabelKosong(tabModeRiwayatPasienRalan);
        try (PreparedStatement psRiwayat=koneksi.prepareStatement(
                "select pr.no_rawat,"+
                "concat(date_format(pr.tgl_perawatan,'%d-%m-%Y'),' ',left(pr.jam_rawat,8)) as tanggal,"+
                "poliklinik.nm_poli,dokter.nm_dokter,ifnull(reg_periksa.stts,'') as status,"+
                "ifnull(penjab.png_jawab,'') as cara_bayar,ifnull(ptg.nama,'') as petugas_input,"+
                ...
                "left join petugas ptg on pr.nip=ptg.nip "+
                "where reg_periksa.no_rkm_medis=? order by pr.tgl_perawatan desc,pr.jam_rawat desc limit 50")) {
            psRiwayat.setString(1,TNoRM.getText());
            try (ResultSet rsRiwayat=psRiwayat.executeQuery()) {
                while(rsRiwayat.next()){
                    String status=nilai(rsRiwayat.getString("status"));
                    if("Sudah".equalsIgnoreCase(status)){ status="Selesai"; }
                    tabModeRiwayatPasienRalan.addRow(new Object[]{
                        rsRiwayat.getString("no_rawat"),
                        rsRiwayat.getString("tanggal"),
                        rsRiwayat.getString("nm_poli"),
                        rsRiwayat.getString("nm_dokter"),
                        status,
                        rsRiwayat.getString("diagnosa"),
                        rsRiwayat.getString("cara_bayar"),
                        rsRiwayat.getString("petugas_input"),
                        rsRiwayat.getString("keluhan"),
                        rsRiwayat.getString("pemeriksaan"),
                        rsRiwayat.getString("penilaian"),
                        rsRiwayat.getString("rtl")
                    });
                }
            }
        } catch (Exception e) {
            System.out.println("Notifikasi Riwayat Data Pasien : "+e);
        }
        if(htmlDetailRiwayatRalan!=null){
            htmlDetailRiwayatRalan.setText("<html><body style='font-family:Tahoma;font-size:11px;color:#888;margin:8px;'>Klik salah satu baris riwayat untuk melihat detail lengkap SOAP.</body></html>");
        }
    }
    ...
    private void pasangTabSBAR(){
        panelSBAR = new rekammedis.PanelSBAR("ralan");
        TabRawat.addTab("SBAR", panelSBAR);
        TabRawat.addChangeListener(new javax.swing.event.ChangeListener(){
            @Override public void stateChanged(javax.swing.event.ChangeEvent e){
                if(TabRawat.getSelectedComponent()==panelSBAR){
                    panelSBAR.setKonteks(TNoRw.getText());
                }
            }
        });
        tambahTombolMenu("Validasi SBAR", new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
                rekammedis.DlgValidasiSBAR d=new rekammedis.DlgValidasiSBAR(null,true,"ralan");
                d.setNoRawat(TNoRw.getText());
                d.setVisible(true);
            }
        });
        tambahTombolMenu("Validasi SOAP", new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
                rekammedis.DlgValidasiSOAP d=new rekammedis.DlgValidasiSOAP(null,true,"ralan");
                d.setNoRawat(TNoRw.getText());
                d.setVisible(true);
            }
        });
        tambahTombolMenu("Cetak CPPT", new java.awt.event.ActionListener(){
            @Override public void actionPerformed(java.awt.event.ActionEvent evt){
    ...
    private void tambahTombolMenu(String teks, java.awt.event.ActionListener aksi){
        ...
        b.setPreferredSize(new java.awt.Dimension(190,23));
        try{ b.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/item.png"))); }catch(Exception e){}
        b.addActionListener(aksi);
        FormMenu.add(b);
        FormMenu.revalidate();
        FormMenu.repaint();
    }
    ...
    // klik/ubah pilihan di tabel SOAP dokter
    tbSoapDokter.getSelectionModel().addListSelectionListener(e -> {
        if (!e.getValueIsAdjusting()) {
            isiSOAPDariTabel(tbSoapDokter);
            tbSoapPerawat.clearSelection(); // biar gak double select
        }
    });

    // klik/ubah pilihan di tabel SOAP perawat
    tbSoapPerawat.getSelectionModel().addListSelectionListener(e -> {
        if (!e.getValueIsAdjusting()) {
            isiSOAPDariTabel(tbSoapPerawat);
            tbSoapDokter.clearSelection();
        }
    });
        ...
        tabModePemeriksaan=new DefaultTableModel(null,new Object[]{
            ...
            "L.P.(Cm)","Plan","Asesmen","Instruksi","Evaluasi","NIP","Dokter/Paramedis","Profesi/Jabatan","Validasi"}){
             @Override public boolean isCellEditable(int rowIndex, int colIndex){
                boolean a = false;
                if (colIndex==0) {
                    a=true;
        ...
        // Buat inputan form SOAPIE (S,O,A,P,I,E) menjadi cetak tebal (bold).
        javax.swing.JTextArea[] soapieBold={TKeluhan,TPemeriksaan,TPenilaian,TindakLanjut,TInstruksi,TEvaluasi};
        for(javax.swing.JTextArea areaSoapie:soapieBold){
            areaSoapie.setFont(areaSoapie.getFont().deriveFont(java.awt.Font.BOLD));
        }
                ...
                if(akses.getform().equals("DlgRawatJalan")){
                    if(dokter.getTable().getSelectedRow()!= -1){
                        if(idxTabRawatRalan()==0){
                            ...
                            KdDok2.requestFocus();
                        }else if(TabRawat.getSelectedComponent()==internalFrame8){
    ...
    private void initComponents() {
        ...
        BtnTemplateSoapExcel = new widget.Button();
        jScrollPane1 = new javax.swing.JScrollPane();
        tbSoapDokter = new javax.swing.JTable();
        jScrollPane2 = new javax.swing.JScrollPane();
        tbSoapPerawat = new javax.swing.JTable();
        ...
        PanelResepTerintegrasi = new widget.PanelBiasa();
        ...
        TabRawat.setPreferredSize(new java.awt.Dimension(879, 1500));
        TabRawat.addMouseListener(new java.awt.event.MouseAdapter() {
        ...
        panelGlass12.setPreferredSize(new java.awt.Dimension(44, 390));
        ...
        jLabel17.setBounds(0, 318, 70, 23);
        ...
        jLabel20.setBounds(130, 318, 90, 23);

        jLabel22.setText("GCS (E,V,M) :");
        jLabel22.setName("jLabel22"); // NOI18N
        panelGlass12.add(jLabel22);
        jLabel22.setBounds(120, 350, 70, 23);
        ...
        scrollPane1.setBounds(73, 70, 360, 72);
        ...
        jLabel26.setBounds(450, 117, 90, 23);
        ...
        scrollPane2.setBounds(73, 149, 360, 92);
        ...
        TTensi.setBounds(223, 288, 74, 23);
        ...
        TTinggi.setBounds(73, 318, 55, 23);
        ...
        TRespirasi.setBounds(223, 318, 55, 23);
        ...
        TBerat.setBounds(378, 288, 55, 23);
        ...
        scrollPane3.setBounds(543, 40, 360, 72);
        ...
        scrollPane6.setBounds(543, 117, 360, 92);

        jLabel29.setText("Kesadaran :");
        jLabel29.setName("jLabel29"); // NOI18N
        panelGlass12.add(jLabel29);
        jLabel29.setBounds(234, 350, 70, 23);
        ...
        cmbKesadaran.setBounds(307, 350, 126, 23);
        ...
        jLabel53.setBounds(450, 216, 90, 23);
        ...
        scrollPane7.setBounds(543, 216, 360, 80);

        jLabel54.setText("SpO2 (%) :");
        jLabel54.setName("jLabel54"); // NOI18N
        panelGlass12.add(jLabel54);
        jLabel54.setBounds(0, 350, 70, 23);
        ...
        SpO2.setBounds(73, 350, 42, 23);

        jLabel56.setText("Evaluasi :");
        jLabel56.setName("jLabel56"); // NOI18N
        panelGlass12.add(jLabel56);
        jLabel56.setBounds(450, 303, 90, 23);
        ...
        scrollPane8.setBounds(543, 303, 360, 72);
        ...
        BtnTemplateSoapExcel.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnTemplateSoapExcelActionPerformed(evt);
            }
        });
        panelGlass12.add(BtnTemplateSoapExcel);
        BtnTemplateSoapExcel.setBounds(436, 40, 28, 23);

        jScrollPane1.setName("jScrollPane1"); // NOI18N

        tbSoapDokter.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        tbSoapDokter.setName("tbSoapDokter"); // NOI18N
        jScrollPane1.setViewportView(tbSoapDokter);

        panelGlass12.add(jScrollPane1);
        jScrollPane1.setBounds(920, 20, 990, 110);

        jScrollPane2.setName("jScrollPane2"); // NOI18N

        tbSoapPerawat.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        tbSoapPerawat.setName("tbSoapPerawat"); // NOI18N
        jScrollPane2.setViewportView(tbSoapPerawat);

        panelGlass12.add(jScrollPane2);
        jScrollPane2.setBounds(922, 142, 990, 230);

        jLabel1.setFont(new java.awt.Font("Serif", 1, 14)); // NOI18N
        jLabel1.setText("SOAP PERAWAT");
        jLabel1.setName("jLabel1"); // NOI18N
        panelGlass12.add(jLabel1);
        jLabel1.setBounds(1930, 190, 110, 19);

        jLabel2.setFont(new java.awt.Font("Serif", 1, 14)); // NOI18N
        jLabel2.setText("SOAP DOKTER");
        jLabel2.setName("jLabel2"); // NOI18N
        panelGlass12.add(jLabel2);
        jLabel2.setBounds(1920, 70, 110, 19);
        ...
        PanelResepTerintegrasi.setName("PanelResepTerintegrasi"); // NOI18N
        PanelResepTerintegrasi.setLayout(new java.awt.BorderLayout(1, 1));

        TabRawat.addTab("Resep", PanelResepTerintegrasi);
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "28-11-2025" }));
        ...
        BtnResume.setText("Resume Rawat Jalan");
                        ...
                        if(KdPeg.getText().trim().equals("")||TPegawai.getText().trim().equals("")){
                            ...
                            if(akses.getkode().equals("Admin Utama")){
                                if(Sequel.menyimpantf("pemeriksaan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Data",25,new String[]{
                                    ...
                                    LingkarPerut.getText(),TindakLanjut.getText(),TPenilaian.getText(),TInstruksi.getText(),TEvaluasi.getText(),KdPeg.getText(),"Belum","",null,null})==true){
                                        sinkronDraftResepSOAP(TNoRw.getText(),Valid.SetTgl(DTPTgl.getSelectedItem()+""),cmbJam.getSelectedItem()+":"+cmbMnt.getSelectedItem()+":"+cmbDtk.getSelectedItem(),KdPeg.getText(),TindakLanjut.getText());
    ...
    private void TabRawatMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_TabRawatMouseClicked
        switch (idxTabRawatRalan()) {
            ...
            case 8:
                ...
                TCari.setPreferredSize(new Dimension(240,23));
                TCariPasien.setText(TNoRM.getText());
                sinkronkanTabResepTerintegrasi(false);
                break;
            case 90:
                BtnSimpan.setEnabled(false);
                BtnHapus.setEnabled(false);
                ...
                TCariPasien.setText(TNoRM.getText());
                tampilDataPasienRalan();
                LCount.setText(""+tabModeRiwayatPasienRalan.getRowCount());
                break;
            case 91:
                BtnSimpan.setEnabled(false);
                BtnHapus.setEnabled(false);
                ...
                case 3:
                    if(KdPeg.getText().trim().equals("")||TPegawai.getText().trim().equals("")){
                        ...
                        if(tbPemeriksaan.getSelectedRow()>-1){
                            String noRawatLama=tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),1).toString();
                            String tglSoapLama=tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),4).toString();
                            String jamSoapLama=tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),5).toString();
                            String kdDokterLama=tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),23).toString();
                            if(akses.getkode().equals("Admin Utama")){
                                if(Sequel.mengedittf("pemeriksaan_ralan","no_rawat='"+tbPemeriksaan.getValueAt(tbPemeriksaan.getSelectedRow(),1)+
                                    ...
                                    "instruksi='"+TInstruksi.getText()+"',evaluasi='"+TEvaluasi.getText()+"',nip='"+KdPeg.getText()+"'")==true){
                                        ubahDraftResepSOAP(noRawatLama,tglSoapLama,jamSoapLama,kdDokterLama,TNoRw.getText(),Valid.SetTgl(DTPTgl.getSelectedItem()+""),cmbJam.getSelectedItem()+":"+cmbMnt.getSelectedItem()+":"+cmbDtk.getSelectedItem(),KdPeg.getText(),TindakLanjut.getText());
...
private void BtnEditKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnEditKeyPressed
    ...
    private void BtnResumeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnResumeActionPerformed
        if(TNoRw.getText().trim().equals("")){
            ...
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            RMResumeMedisRalanV2 resume=new RMResumeMedisRalanV2(null,false);
            resume.isCek();
    ...
    private void BtnTemplateSoapExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnTemplateSoapExcelActionPerformed
        if(TPasien.getText().trim().equals("")||TNoRw.getText().trim().equals("")){
            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu dengan menklik data pada table...!!!");
            TCari.requestFocus();
        }else{
            String lokasiTemplate=koneksiDB.LOKASITEMPLATESOAPCSV().trim();
            if(lokasiTemplate.isEmpty()){
                JOptionPane.showMessageDialog(null,"Lokasi file template SOAP CSV belum diatur.\nSilahkan isi key LOKASITEMPLATESOAPCSV di file setting/database.xml");
                return;
            }
            File fileTemplate = new File(lokasiTemplate);
            if(!fileTemplate.exists()){
                JOptionPane.showMessageDialog(null,"File template SOAP CSV tidak ditemukan :\n"+lokasiTemplate);
            }else{
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                DlgTemplateSOAPExcel dialog = new DlgTemplateSOAPExcel(null,true,lokasiTemplate);
    ...
    }//GEN-LAST:event_BtnTemplateSoapExcelActionPerformed
    ...
    */
    ...
    private widget.Button BtnTemplateSoapExcel;
    ...
    private widget.PanelBiasa PanelResepTerintegrasi;
    ...
    private javax.swing.JTable tbSoapDokter;
    private javax.swing.JTable tbSoapPerawat;
    private widget.Table tbTindakan;
    private widget.Table tbTindakan2;
    private widget.Table tbTindakan3;
    // End of variables declaration//GEN-END:variables
    private widget.Button BtnSkorBromagePascaAnestesi,BtnPenilaianPreInduksi,BtnHasilPemeriksaanUSGUrologi,BtnHasilPemeriksaanUSGGynecologi,BtnHasilPemeriksaanEKG,BtnPenatalaksanaanTerapiOkupasi,
                            //BtnPenilaianPsikolog,
                          BtnHasilPemeriksaanUSGNeonatus,BtnHasilEndoskopiFaringLaring;

    private void sanitizeInputSOAP() {
        Valid.sanitizeTextArea(TKeluhan);
        Valid.sanitizeTextArea(TPemeriksaan);
        Valid.sanitizeTextArea(TPenilaian);
        Valid.sanitizeTextArea(TindakLanjut);
        Valid.sanitizeTextArea(TInstruksi);
        Valid.sanitizeTextArea(TEvaluasi);
        TAlergi.setText(Valid.sanitizeWhatsAppText(TAlergi.getText()));
        LingkarPerut.setText(Valid.sanitizeWhatsAppText(LingkarPerut.getText()));
    }
    ...
    public void setNoRm(String norwt,Date tgl1,Date tgl2) {
        ...
        ChkInput3.setSelected(true);
        isForm4();
        konteksResepTerintegrasi="";
        sinkronkanTabResepTerintegrasi(true);
...
//    // kosongkan tabel & SOAP dulu
//    Valid.tabelKosong(tabModePemeriksaan);
//    TSoapDokter.setText("");
//    TSoapPerawat.setText("");
//
//    StringBuilder soapDokter  = new StringBuilder();
//    StringBuilder soapPerawat = new StringBuilder();
//
//    try {
...
//            "where pemeriksaan_ralan.tgl_perawatan between ? and ? and reg_periksa.no_rkm_medis like ? "+
...
//sb.append("Tgl ").append(rs.getString("tgl_perawatan"))
//  .append(" Jam ").append(rs.getString("jam_rawat"))
//  .append(" / ").append(rs.getString("nama"))
//  .append(" (").append(rs.getString("jbtn")).append(")\n");
//
//sb.append("S : ").append(nvl(rs.getString("keluhan"))).append("\n");
//sb.append("O : TD ").append(nvl(rs.getString("tensi")))
//  .append(", N ").append(nvl(rs.getString("nadi")))
//  .append(", RR ").append(nvl(rs.getString("respirasi")))
//  .append(", Suhu ").append(nvl(rs.getString("suhu_tubuh")))
//  .append(", BB ").append(nvl(rs.getString("berat")))
//  .append(", TB ").append(nvl(rs.getString("tinggi")))
//  .append(", SpO2 ").append(nvl(rs.getString("spo2")))
//  .append(", GCS ").append(nvl(rs.getString("gcs")))
//  .append(", Kesadaran ").append(nvl(rs.getString("kesadaran")))
//  .append("\n");
//sb.append("A : ").append(nvl(rs.getString("penilaian"))).append("\n");
//sb.append("P : ").append(nvl(rs.getString("rtl"))).append("\n");
//
//String instruksi = nvl(rs.getString("instruksi"));
//if (!instruksi.equals("")) {
//    sb.append("Instruksi : ").append(instruksi).append("\n");
//}
//
//String evaluasi = nvl(rs.getString("evaluasi"));
//if (!evaluasi.equals("")) {
//    sb.append("Evaluasi : ").append(evaluasi).append("\n");
//}
//sb.append("\n");
//
//// === PEMISAH DOKTER VS PERAWAT (WAJIB pakai if - else if - else) ===
//String namaPetugas = nvl(rs.getString("nama")).trim().toLowerCase();
//
//// nama diawali dr / dr. / drg / drg.
//boolean isDokter = namaPetugas.matches("(?i)^(dr|dr\\.|drg|drg\\.)\\b.*");
//
//if (isDokter) {
//    // masuk ke text area SOAP dokter
//    soapDokter.append(sb);
//} else {
//    // sisanya masuk ke SOAP perawat
//    soapPerawat.append(sb);
//}
//            }
//        } catch (Exception e) {
//            System.out.println("Notifikasi : " + e);
...
//            }
//            if (ps4 != null) {
//                try { ps4.close(); } catch (Exception ex) {}
//            }
//        }
//    } catch (Exception e) {
//        System.out.println("Notifikasi : " + e);
...
//    TSoapDokter.setText(soapDokter.toString());
//    TSoapPerawat.setText(soapPerawat.toString());
//
//    LCount.setText(String.valueOf(tabModePemeriksaan.getRowCount()));
//}

// helper kecil biar gak null pointer
...
//    }
    ...
    private void initTabResepTerintegrasi() {
        ...
        PanelResepTerintegrasi.repaint();
    }
    ...
    public void aktifkanModeEmbeddedPemeriksaan() {
        modeEmbeddedPemeriksaan=true;
        BtnKeluar.setVisible(false);
        internalFrame1.setBorder(null);
        PanelAccor.setVisible(false);
        PanelAccor.setPreferredSize(new Dimension(0,0));
    ...
    public void fokusTabPemeriksaanTerintegrasi() {
        TabRawat.setSelectedComponent(internalFrame5);
        TabRawatMouseClicked(null);
    }

    private void sinkronkanTabResepTerintegrasi(boolean paksa) {
        if(resepTerintegrasiRalan==null){
            return;
        }
        String konteksBaru=TNoRw.getText()+"|"+Valid.SetTgl(DTPTgl.getSelectedItem()+"")+"|"+cmbJam.getSelectedItem()+"|"+cmbMnt.getSelectedItem()+"|"+cmbDtk.getSelectedItem()+"|"+KdDok.getText();
        if(!paksa && konteksBaru.equals(konteksResepTerintegrasi)){
            return;
        }
        konteksResepTerintegrasi=konteksBaru;
        if(TNoRw.getText().trim().equals("") || TPasien.getText().trim().equals("")){
            return;
        }
        resepTerintegrasiRalan.setNoRm(
                TNoRw.getText(),
                DTPTgl.getDate(),
        ...
        resepTerintegrasiRalan.isCek();
        resepTerintegrasiRalan.tampilobat();
        resepTerintegrasiRalan.emptTeksobat();
    }

    private void inputObat() {
        DlgCariObat dlgobt=DlgCariObat.buatDari(this);
    ...
    private void inputResep() {
        sinkronkanTabResepTerintegrasi(false);
        TabRawat.setSelectedComponent(PanelResepTerintegrasi);
        TabRawatMouseClicked(null);
    }
    ...
    private void pastikanTabelDraftResepSOAP() {
        ...
        try {
            ...
            System.out.println("Notif Draft Resep SOAP : "+e);
        }
    }

    private boolean bolehJadiDraftResepSOAP(String kdDokter,String resepTeks) {
        return !kdDokter.trim().equals("") && !resepTeks.trim().equals("") &&
                !Sequel.cariIsi("select nm_dokter from dokter where kd_dokter=?",kdDokter).trim().equals("");
    }

    private void hapusDraftResepSOAPPending(String noRawat,String tglSoap,String jamSoap,String kdDokter) {
        if(noRawat.trim().equals("")||tglSoap.trim().equals("")||jamSoap.trim().equals("")||kdDokter.trim().equals("")){
            return;
        }
    ...
    private void sinkronDraftResepSOAP(String noRawat,String tglSoap,String jamSoap,String kdDokter,String resepTeks) {
        hapusDraftResepSOAPPending(noRawat,tglSoap,jamSoap,kdDokter);
        if(!bolehJadiDraftResepSOAP(kdDokter,resepTeks)){
            return;
        }
        pastikanTabelDraftResepSOAP();
        Sequel.queryu2("insert into permintaan_resep_soap ("+
                "no_rawat,tgl_soap,jam_soap,kd_dokter,tgl_permintaan,jam_permintaan,resep_teks,status,no_resep"+
                ") values (?,?,?,?,?,?,?,?,?)",9,new String[]{
            noRawat,tglSoap,jamSoap,kdDokter,tglSoap,jamSoap,resepTeks,"Belum Terlayani",""
        });
    }

    private void ubahDraftResepSOAP(String noRawatLama,String tglSoapLama,String jamSoapLama,String kdDokterLama,
            String noRawatBaru,String tglSoapBaru,String jamSoapBaru,String kdDokterBaru,String resepTeksBaru) {
        hapusDraftResepSOAPPending(noRawatLama,tglSoapLama,jamSoapLama,kdDokterLama);
        sinkronDraftResepSOAP(noRawatBaru,tglSoapBaru,jamSoapBaru,kdDokterBaru,resepTeksBaru);
    }
   ...
   private void tampilPemeriksaan() {
    ...
    Valid.tabelKosong(tabModePemeriksaan);
    // kosongkan tabel SOAP dokter & perawat (kalau sudah dibuat)
    if (tabModeSoapDokter != null) {
        Valid.tabelKosong(tabModeSoapDokter);
    }
    if (tabModeSoapPerawat != null) {
        Valid.tabelKosong(tabModeSoapPerawat);
    }
    ...
    try {
        ps4 = koneksi.prepareStatement(
            ...
            "pemeriksaan_ralan.nip,pegawai.nama,pegawai.jbtn,pemeriksaan_ralan.validasi " +
            "from pasien inner join reg_periksa on reg_periksa.no_rkm_medis=pasien.no_rkm_medis " +
            "inner join pemeriksaan_ralan on pemeriksaan_ralan.no_rawat=reg_periksa.no_rawat " +
            "inner join pegawai on pemeriksaan_ralan.nip=pegawai.nik " +
            "where pemeriksaan_ralan.tgl_perawatan between ? and ? and reg_periksa.no_rkm_medis like ? " +
        ...
        try {
            ...
            rs = ps4.executeQuery();
            while (rs.next()) {
                // ====== TABEL PEMERIKSAAN (ASLI) ======
                tabModePemeriksaan.addRow(new Object[]{
                    false,
                    rs.getString(1),  // no_rawat
                    rs.getString(2),  // no_rkm_medis
                    rs.getString(3),  // nm_pasien
                    rs.getString(4),  // tgl_perawatan
                    rs.getString(5),  // jam_rawat
                    rs.getString(6),  // suhu_tubuh
                    rs.getString(7),  // tensi
                    rs.getString(8),  // nadi
                    rs.getString(9),  // respirasi
                    rs.getString(10), // tinggi
                    rs.getString(11), // berat
                    rs.getString(12), // spo2
                    rs.getString(13), // gcs
                    rs.getString(14), // kesadaran
                    rs.getString(15), // keluhan
                    rs.getString(16), // pemeriksaan
                    rs.getString(17), // alergi
                    rs.getString(18), // lingkar_perut
                    rs.getString(19), // rtl
                    rs.getString(20), // penilaian
                    rs.getString(21), // instruksi
                    rs.getString(22), // evaluasi
                    rs.getString(23), // nip
                    rs.getString(24), // nama
                    rs.getString(25), // jbtn
                    (rs.getString(26) != null && rs.getString(26).equalsIgnoreCase("Sudah")) ? "Sudah" : "Belum" // validasi
                });

                // ====== TABEL SOAP DOKTER & PERAWAT ======
                if (tabModeSoapDokter != null && tabModeSoapPerawat != null) {
                    String tgl     = rs.getString("tgl_perawatan");
                    String jam     = rs.getString("jam_rawat");
                    String petugas = nvl(rs.getString("nama"));

                    // TTV dipisah ke kolom sendiri
                    String ttv = "TD " + nvl(rs.getString("tensi")) +
                                 ", N " + nvl(rs.getString("nadi")) +
                                 ", RR " + nvl(rs.getString("respirasi")) +
                                 ", Suhu " + nvl(rs.getString("suhu_tubuh")) +
                                 ", BB " + nvl(rs.getString("berat")) +
                                 ", TB " + nvl(rs.getString("tinggi")) +
                                 ", SpO2 " + nvl(rs.getString("spo2")) +
                                 ", GCS " + nvl(rs.getString("gcs")) +
                                 ", Kesadaran " + nvl(rs.getString("kesadaran"));

                    String S         = nvl(rs.getString("keluhan"));       // Subjektif
                    String O         = nvl(rs.getString("pemeriksaan"));   // Objek (yang di kolom Objek bawah)
                    String A         = nvl(rs.getString("penilaian"));
                    String P         = nvl(rs.getString("rtl"));
                    String instruksi = nvl(rs.getString("instruksi"));
                    String evaluasi  = nvl(rs.getString("evaluasi"));

                    Object[] rowSOAP = new Object[]{
                        tgl, jam, petugas, ttv, S, O, A, P, instruksi, evaluasi
                    };

                    // klasifikasi dokter vs perawat dari nama (dr/dr./drg/drg.)
                    String namaPetugas = petugas.trim().toLowerCase();
                    boolean isDokter = namaPetugas.matches("(?i)^(dr|dr\\.|drg|drg\\.)\\b.*");

                    if (isDokter) {
                        tabModeSoapDokter.addRow(rowSOAP);
                    } else {
                        tabModeSoapPerawat.addRow(rowSOAP);
                    }
                }
            }
        } catch (Exception e) {
            ...
            if (ps4 != null) {
                try { ps4.close(); } catch (Exception ex) { /* ignore */ }
            }
        }
    } catch (Exception e) {
        System.out.println("Notifikasi : " + e);
    ...
    private void initTableSOAP() {
    tabModeSoapDokter = new DefaultTableModel(null, new Object[]{
        "Tgl", "Jam", "Petugas", "TTV", "S", "O", "A", "P", "Instruksi", "Evaluasi"
    }) {
    ...
    tbSoapDokter.setModel(tabModeSoapDokter);

    tabModeSoapPerawat = new DefaultTableModel(null, new Object[]{
        "Tgl", "Jam", "Petugas", "TTV", "S", "O", "A", "P", "Instruksi", "Evaluasi"
    }) {
        @Override public boolean isCellEditable(int row, int column) {
            return false;
    ...
    tbSoapPerawat.setModel(tabModeSoapPerawat);

    // dst: style header, rowHeight, dll (tetap seperti yang kemarin)
    styleHeader(tbSoapDokter);
    styleHeader(tbSoapPerawat);

    tbSoapDokter.setRowHeight(26);
    tbSoapPerawat.setRowHeight(26);
    tbSoapDokter.setShowHorizontalLines(false);
    tbSoapPerawat.setShowHorizontalLines(false);
    tbSoapDokter.setShowVerticalLines(false);
    tbSoapPerawat.setShowVerticalLines(false);
    tbSoapDokter.setIntercellSpacing(new java.awt.Dimension(0, 0));
    tbSoapPerawat.setIntercellSpacing(new java.awt.Dimension(0, 0));
    
    // nanti renderer kita ganti di langkah 3
    SoapCellRenderer soapRenderer = new SoapCellRenderer();

// pakai untuk semua kolom, biar wrap & selang-seling
for (int i = 0; i < tbSoapDokter.getColumnCount(); i++) {
    tbSoapDokter.getColumnModel().getColumn(i).setCellRenderer(soapRenderer);
}
for (int i = 0; i < tbSoapPerawat.getColumnCount(); i++) {
    tbSoapPerawat.getColumnModel().getColumn(i).setCellRenderer(soapRenderer);
}
}

    private boolean soapListenerDipasang = false;
    ...
    private void pasangTabLaborat() {
        LoadHTMLLaborat = new widget.editorpane();
        LoadHTMLLaborat.setContentType("text/html");
        LoadHTMLLaborat.setEditable(false);
        LoadHTMLLaborat.setBorder(null);
        widget.ScrollPane sc = new widget.ScrollPane();
        sc.setViewportView(LoadHTMLLaborat);
        final widget.InternalFrame frame = new widget.InternalFrame();
        frame.setLayout(new java.awt.BorderLayout(1, 1));
        frame.add(sc, java.awt.BorderLayout.CENTER);
        TabRawat.addTab("Laboratorium", frame);
        TabRawat.addChangeListener(new javax.swing.event.ChangeListener() {
            @Override public void stateChanged(javax.swing.event.ChangeEvent e) {
                if (TabRawat.getSelectedComponent() == frame) {
                    tampilLaboratPasien();
                }
            }
        });
        tampilLaboratKosong("Pilih pasien atau klik Cari untuk menampilkan data laboratorium.");
    }

    private void tampilLaboratKosong(String pesan) {
        LoadHTMLLaborat.setText(
            "<html><head><style>" +
            "body{font-family:tahoma;font-size:11px;color:#333;background:#fff;margin:10px;}" +
            ".info{padding:10px;border:1px solid #d8e7d8;background:#f8fff8;}" +
            "</style></head><body><div class='info'>" + html(pesan) + "</div></body></html>"
        );
        LoadHTMLLaborat.setCaretPosition(0);
    }

    private void tampilLaboratPasien() {
        String noRM = TNoRM.getText().trim();
        if (noRM.equals("")) {
            noRM = TCariPasien.getText().trim();
        }
        if (noRM.equals("") && !TNoRw.getText().trim().equals("")) {
            noRM = Sequel.cariIsi("select reg_periksa.no_rkm_medis from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText().trim());
        }
        if (noRM.equals("")) {
            tampilLaboratKosong("Pilih pasien dulu untuk menampilkan hasil laboratorium.");
            return;
        }
        StringBuilder htmlContent = new StringBuilder();
        htmlContent.append("<html><head><style>")
                .append("body{font-family:tahoma;font-size:11px;color:#333;background:#fff;margin:8px;}")
                .append("table{width:100%;border-collapse:collapse;margin-bottom:10px;}")
                .append("th{background:#FFFAF8;border:1px solid #e2e7dd;padding:4px;text-align:center;}")
                .append("td{border:1px solid #e2e7dd;padding:4px;vertical-align:top;}")
                .append(".judul{font-weight:bold;background:#edf7ed;padding:6px;border:1px solid #d8e7d8;margin:8px 0 0 0;}")
                .append(".subjudul{background:#fff8f4;font-weight:bold;}")
                .append(".muted{color:#666;}")
                .append("</style></head><body>")
                .append("<div class='muted'>No.RM: ").append(html(noRM)).append("</div>");
        int total = 0;
        total += appendLaboratPkMb(htmlContent, noRM);
        total += appendLaboratPa(htmlContent, noRM);
        if (total == 0) {
            htmlContent.append("<div class='judul'>Data laboratorium tidak ditemukan untuk pasien ini.</div>");
        }
        htmlContent.append("</body></html>");
        LoadHTMLLaborat.setText(htmlContent.toString());
        LoadHTMLLaborat.setCaretPosition(0);
    }

    private int appendLaboratPkMb(StringBuilder htmlContent, String noRM) {
        int nomor = 0;
        try (PreparedStatement psGrup = koneksi.prepareStatement(
                "select periksa_lab.no_rawat,periksa_lab.tgl_periksa,periksa_lab.jam "+
                "from periksa_lab inner join reg_periksa on periksa_lab.no_rawat=reg_periksa.no_rawat "+
                "where periksa_lab.kategori<>'PA' and reg_periksa.no_rkm_medis=? "+
                "group by periksa_lab.no_rawat,periksa_lab.tgl_periksa,periksa_lab.jam "+
                "order by periksa_lab.tgl_periksa,periksa_lab.jam")) {
            psGrup.setString(1, noRM);
            try (ResultSet rsGrup = psGrup.executeQuery()) {
                while (rsGrup.next()) {
                    if (nomor == 0) {
    ...
    private void appendDetailLaboratPkMb(StringBuilder htmlContent, int nomor, String noRawat, String tglPeriksa, String jamPeriksa) {
        try (PreparedStatement psItem = koneksi.prepareStatement(
                "select periksa_lab.kd_jenis_prw,jns_perawatan_lab.nm_perawatan,petugas.nama,periksa_lab.biaya,dokter.nm_dokter "+
                ...
                "where periksa_lab.kategori<>'PA' and periksa_lab.no_rawat=? and periksa_lab.tgl_periksa=? and periksa_lab.jam=? "+
                "order by periksa_lab.kd_jenis_prw")) {
            psItem.setString(1, noRawat);
            psItem.setString(2, tglPeriksa);
            psItem.setString(3, jamPeriksa);
            int urutItem = 0;
            try (ResultSet rsItem = psItem.executeQuery()) {
                while (rsItem.next()) {
                    urutItem++;
                    htmlContent.append("<tr>")
                            .append("<td align='center'>").append(urutItem == 1 ? nomor : "").append("</td>")
                            .append("<td>").append(urutItem == 1 ? html(noRawat) : "").append("</td>")
                            .append("<td>").append(urutItem == 1 ? html(tglPeriksa) + " " + html(jamPeriksa) : "").append("</td>")
                            .append("<td>").append(html(rsItem.getString("kd_jenis_prw"))).append("</td>")
            ...
            appendSaranKesanLaborat(htmlContent, noRawat, tglPeriksa, jamPeriksa);
            appendLaporKritisLaborat(htmlContent, noRawat, tglPeriksa, jamPeriksa);
        } catch (Exception e) {
            System.out.println("Notifikasi Lab : " + e);
        }
    }

    private void appendNilaiLaboratPkMb(StringBuilder htmlContent, String noRawat, String kdJenisPrw, String tglPeriksa, String jamPeriksa) {
        try (PreparedStatement psNilai = koneksi.prepareStatement(
            ...
            psNilai.setString(1, noRawat);
            psNilai.setString(2, kdJenisPrw);
            psNilai.setString(3, tglPeriksa);
            psNilai.setString(4, jamPeriksa);
    ...
    private void appendSaranKesanLaborat(StringBuilder htmlContent, String noRawat, String tglPeriksa, String jamPeriksa) {
        try (PreparedStatement psSaran = koneksi.prepareStatement(
                "select saran,kesan from saran_kesan_lab where no_rawat=? and tgl_periksa=? and jam=?")) {
            psSaran.setString(1, noRawat);
            psSaran.setString(2, tglPeriksa);
            psSaran.setString(3, jamPeriksa);
            try (ResultSet rsSaran = psSaran.executeQuery()) {
                if (rsSaran.next()) {
                    htmlContent.append("<tr><td></td><td></td><td>Kesan</td><td colspan='5'>: ").append(br(rsSaran.getString("kesan"))).append("</td></tr>")
                            .append("<tr><td></td><td></td><td>Saran</td><td colspan='5'>: ").append(br(rsSaran.getString("saran"))).append("</td></tr>");
                }
            }
        } catch (Exception e) {
            System.out.println("Notif : " + e);
    ...
    /** Jika pemeriksaan punya nilai kritis (keterangan 'K'), tampilkan jam lapor &
     *  jam respon Dokter PJ dari lapor_kritis_lab di bagian bawah blok lab. */
    private void appendLaporKritisLaborat(StringBuilder htmlContent, String noRawat, String tglPeriksa, String jamPeriksa) {
        try {
            boolean adaKritis = false;
            try (PreparedStatement ps = koneksi.prepareStatement(
                    "select count(*) from detail_periksa_lab where no_rawat=? and tgl_periksa=? and jam=? and trim(keterangan)='K'")) {
                ps.setString(1, noRawat);
                ps.setString(2, tglPeriksa);
                ps.setString(3, jamPeriksa);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) { adaKritis = rs.getInt(1) > 0; }
                }
            }
            if (!adaKritis) {
                return;
            ...
            try (PreparedStatement ps = koneksi.prepareStatement(
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        jamLapor = (jl == null || jl.startsWith("0000") || jl.length() < 19) ? "" : jl.substring(0, 19);
                        jamRespon = (jr == null || jr.startsWith("0000") || jr.length() < 19) ? "" : jr.substring(0, 19);
                    }
                }
            }
            htmlContent.append("<tr><td></td><td></td><td><font color='red'><b>Nilai Kritis</b></font></td><td colspan='5'><font color='red'>: Perlu pelaporan ke Dokter PJ</font></td></tr>")
                    .append("<tr><td></td><td></td><td>Jam Lapor Dokter PJ</td><td colspan='5'>: ").append(jamLapor.equals("") ? "<i>belum diisi</i>" : html(jamLapor)).append("</td></tr>")
                    .append("<tr><td></td><td></td><td>Jam Respon Dokter PJ</td><td colspan='5'>: ").append(jamRespon.equals("") ? "<i>belum diisi</i>" : html(jamRespon)).append("</td></tr>");
        } catch (Exception e) {
            System.out.println("Notif lapor kritis lab : " + e);
        }
    }

    private int appendLaboratPa(StringBuilder htmlContent, String noRM) {
        int nomor = 0;
        try (PreparedStatement psPa = koneksi.prepareStatement(
                "select periksa_lab.no_rawat,periksa_lab.tgl_periksa,periksa_lab.jam,periksa_lab.kd_jenis_prw,"+
                "jns_perawatan_lab.nm_perawatan,petugas.nama,periksa_lab.biaya,dokter.nm_dokter "+
                "from periksa_lab inner join reg_periksa on periksa_lab.no_rawat=reg_periksa.no_rawat "+
                "inner join jns_perawatan_lab on periksa_lab.kd_jenis_prw=jns_perawatan_lab.kd_jenis_prw "+
                "inner join petugas on periksa_lab.nip=petugas.nip inner join dokter on periksa_lab.kd_dokter=dokter.kd_dokter "+
                "where periksa_lab.kategori='PA' and reg_periksa.no_rkm_medis=? "+
                "order by periksa_lab.tgl_periksa,periksa_lab.jam")) {
            psPa.setString(1, noRM);
            try (ResultSet rsPa = psPa.executeQuery()) {
                while (rsPa.next()) {
                    if (nomor == 0) {
                        htmlContent.append("<div class='judul'>Pemeriksaan Laboratorium PA</div><table>")
                    ...
                    nomor++;
                    htmlContent.append("<tr><td align='center'>").append(nomor).append("</td>")
                            .append("<td>").append(html(rsPa.getString("no_rawat"))).append("</td>")
                            .append("<td>").append(html(rsPa.getString("tgl_periksa"))).append(" ").append(html(rsPa.getString("jam"))).append("</td>")
                            .append("<td>").append(html(rsPa.getString("kd_jenis_prw"))).append("</td>")
                            .append("<td>").append(html(rsPa.getString("nm_perawatan"))).append("</td>")
                            .append("<td>").append(html(rsPa.getString("nm_dokter"))).append("</td>")
                            .append("<td>").append(html(rsPa.getString("nama"))).append("</td>")
                            .append("<td align='right'>").append(Valid.SetAngka(rsPa.getDouble("biaya"))).append("</td></tr>");
                    appendDetailLaboratPa(htmlContent, rsPa.getString("no_rawat"), rsPa.getString("kd_jenis_prw"), rsPa.getString("tgl_periksa"), rsPa.getString("jam"));
                }
            }
        } catch (Exception e) {
            System.out.println("Notifikasi Lab : " + e);
    ...
    private void appendDetailLaboratPa(StringBuilder htmlContent, String noRawat, String kdJenisPrw, String tglPeriksa, String jamPeriksa) {
        try (PreparedStatement psDetailPa = koneksi.prepareStatement(
            ...
            psDetailPa.setString(1, noRawat);
            psDetailPa.setString(2, kdJenisPrw);
            psDetailPa.setString(3, tglPeriksa);
            psDetailPa.setString(4, jamPeriksa);
            try (ResultSet rsDetailPa = psDetailPa.executeQuery()) {
                if (rsDetailPa.next()) {
                    htmlContent.append("<tr><td></td><td></td><td>Diagnosa Klinis</td><td colspan='5'>: ").append(br(rsDetailPa.getString("diagnosa_klinik"))).append("</td></tr>")
                            .append("<tr><td></td><td></td><td>Makroskopik</td><td colspan='5'>: ").append(br(rsDetailPa.getString("makroskopik"))).append("</td></tr>")
                            .append("<tr><td></td><td></td><td>Mikroskopik</td><td colspan='5'>: ").append(br(rsDetailPa.getString("mikroskopik"))).append("</td></tr>")
                            .append("<tr><td></td><td></td><td>Kesimpulan</td><td colspan='5'>: ").append(br(rsDetailPa.getString("kesimpulan"))).append("</td></tr>")
                            .append("<tr><td></td><td></td><td>Kesan</td><td colspan='5'>: ").append(br(rsDetailPa.getString("kesan"))).append("</td></tr>");
                }
            }
            appendGambarLaboratPa(htmlContent, noRawat, kdJenisPrw, tglPeriksa, jamPeriksa);
        } catch (Exception e) {
            System.out.println("Notifikasi : " + e);
        }
    }

    private void appendGambarLaboratPa(StringBuilder htmlContent, String noRawat, String kdJenisPrw, String tglPeriksa, String jamPeriksa) {
        try (PreparedStatement psGambarPa = koneksi.prepareStatement(
                "select photo from detail_periksa_labpa_gambar where no_rawat=? and kd_jenis_prw=? and tgl_periksa=? and jam=?")) {
            psGambarPa.setString(1, noRawat);
            psGambarPa.setString(2, kdJenisPrw);
            psGambarPa.setString(3, tglPeriksa);
            psGambarPa.setString(4, jamPeriksa);
            try (ResultSet rsGambarPa = psGambarPa.executeQuery()) {
                while (rsGambarPa.next()) {
                    if (!nilai(rsGambarPa.getString("photo")).equals("")) {
                        String urlGambar = urlGambarLaboratPa(rsGambarPa.getString("photo"));
                        htmlContent.append("<tr><td></td><td></td><td colspan='6' align='center'>")
                                .append("<img alt='Gambar PA' src='").append(html(urlGambar)).append("' width='450' height='450'/><br>")
                                .append(html(rsGambarPa.getString("photo"))).append("</td></tr>");
                    }
                }
            }
        } catch (Exception e) {
    ...
    private String urlGambarLaboratPa(String lokasiGambar) {
        return "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/labpa/" + nilai(lokasiGambar);
    }

    private String br(String text) {
        return html(text).replaceAll("(\\r\\n|\\r|\\n|\\n\\r)", "<br>");
    }

    private String html(String text) {
        String hasil = nilai(text);
    ...
    private void rapikanPanelInputSOAP() {
        Font fontInputSOAP = new Font("Tahoma", Font.PLAIN, 15);
        TKeluhan.setFont(fontInputSOAP);
        TPemeriksaan.setFont(fontInputSOAP);
        TPenilaian.setFont(fontInputSOAP);
        TindakLanjut.setFont(fontInputSOAP);
        TInstruksi.setFont(fontInputSOAP);
        TEvaluasi.setFont(fontInputSOAP);

        Font fontJudulSOAP = new Font("Tahoma", Font.BOLD, 12);
        jLabel8.setFont(fontJudulSOAP);   // Subjek
        jLabel9.setFont(fontJudulSOAP);   // Objek
        jLabel28.setFont(fontJudulSOAP);  // Asesmen
        jLabel26.setFont(fontJudulSOAP);  // Plan
        jLabel53.setFont(fontJudulSOAP);  // Instruksi
        jLabel56.setFont(fontJudulSOAP);  // Evaluasi
    ...
    /** Tata letak input SOAPIE 3 kolom + TTV + tabel riwayat SOAP di bawahnya,
     *  responsif mengikuti lebar/tinggi panelGlass12 (memaksimalkan ruang). */
    private void susunLayoutSoapie3Kolom() {
        int W = panelGlass12.getWidth();
        int H = panelGlass12.getHeight();
        if (W < 200) { W = 1860; }
        if (H < 200) { H = 440; }
        int M = 10, gap = 12, lblH = 20, fh = 23;

        // ---- Baris atas: Dilakukan, Profesi/Jabatan/Departemen, Alergi ----
        int hy = 8;
        int x = M;
        jLabel37.setBounds(x, hy, 70, fh); x += 72;
        KdPeg.setBounds(x, hy, 80, fh); x += 84;
        TPegawai.setBounds(x, hy, 175, fh); x += 179;
        BtnSeekPegawai.setBounds(x, hy, 28, fh); x += 36;
        jLabel41.setBounds(x, hy, 185, fh); x += 188;
        Jabatan.setBounds(x, hy, 165, fh); x += 169;
        ...
        // ---- Geometri band bawah (TTV, 2 baris) ----
        int bottomMargin = 10;
        int ttvBottom = H - bottomMargin;
        int ttvTop = ttvBottom - 62;

        // ---- 3 kolom area teks ----
        int colsTop = 40;
        int colsBottom = ttvTop - 10;
        int colsH = colsBottom - colsTop;
        if (colsH < 150) { colsH = 150; }
        int cellH = (colsH - 2 * lblH - gap) / 2;
        if (cellH < 55) { cellH = 55; }
        int colW = (W - 2 * M - 2 * gap) / 3;
        int c1 = M, c2 = M + colW + gap, c3 = M + 2 * (colW + gap);
        int r1y = colsTop;
        int r2y = colsTop + lblH + cellH + gap;

        jLabel8.setBounds(c1, r1y, colW, lblH);  jLabel8.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane1.setBounds(c1, r1y + lblH, colW, cellH);
        jLabel9.setBounds(c1, r2y, colW, lblH);  jLabel9.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane2.setBounds(c1, r2y + lblH, colW, cellH);

        jLabel28.setBounds(c2, r1y, colW, lblH); jLabel28.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane3.setBounds(c2, r1y + lblH, colW, cellH);
        jLabel26.setBounds(c2, r2y, colW, lblH); jLabel26.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane6.setBounds(c2, r2y + lblH, colW, cellH);

        jLabel53.setBounds(c3, r1y, colW, lblH); jLabel53.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane7.setBounds(c3, r1y + lblH, colW, cellH);
        jLabel56.setBounds(c3, r2y, colW, lblH); jLabel56.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        scrollPane8.setBounds(c3, r2y + lblH, colW, cellH);

        // ---- TTV (grid rapi: 2 baris, SpO2/GCS pindah ke kanan) ----
        int t1 = ttvTop, t2 = ttvTop + 30;
        int lw = 95, fo = lw + 3;                 // lebar label & jarak ke field
        int la = M, lb = M + 185, lc = M + 370, ld = M + 555, le = M + 740;
        int fa = la + fo, fb = lb + fo, fc = lc + fo, fd = ld + fo, fe = le + 78;

        jLabel7.setBounds(la, t1, lw, fh);  TSuhu.setBounds(fa, t1, 55, fh);
        jLabel4.setBounds(lb, t1, lw, fh);  TTensi.setBounds(fb, t1, 74, fh);
    ...
    private void siapkanNotifValidasi() {
        if (notifValidasiSiap) {
            return;
        }
        notifValidasiSiap = true;
        for (javax.swing.JLabel lbl : new javax.swing.JLabel[]{lblNotifSBAR, lblNotifSOAP}) {
            lbl.setOpaque(true);
            lbl.setBorder(javax.swing.BorderFactory.createCompoundBorder(
                    javax.swing.BorderFactory.createLineBorder(new java.awt.Color(200, 120, 0)),
                    javax.swing.BorderFactory.createEmptyBorder(2, 8, 2, 8)));
            lbl.setBackground(new java.awt.Color(255, 244, 210));
            lbl.setForeground(new java.awt.Color(160, 80, 0));
            lbl.setFont(new java.awt.Font("Tahoma", java.awt.Font.BOLD, 12));
            lbl.setCursor(java.awt.Cursor.getPredefinedCursor(java.awt.Cursor.HAND_CURSOR));
            lbl.setVisible(false);
            panelGlass12.add(lbl);
        }
        lblNotifSBAR.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
        ...
        lblNotifSOAP.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent e) {
                rekammedis.DlgValidasiSOAP d = new rekammedis.DlgValidasiSOAP(null, true, "ralan");
                d.setNoRawat(TNoRw.getText());
    ...
    /** Cek pending validasi SBAR & SOAP untuk pasien aktif; tampil hanya untuk dokter. */
    private void refreshNotifValidasi() {
        siapkanNotifValidasi();
        String norw = TNoRw.getText().trim();
        String nmDok = Sequel.cariIsi("select nm_dokter from dokter where kd_dokter=?", akses.getkode());
        boolean dokter = nmDok != null && !nmDok.trim().equals("");
        if (!dokter || norw.equals("")) {
            lblNotifSBAR.setVisible(false);
            lblNotifSOAP.setVisible(false);
            return;
        }
        int nSbar = rekammedis.DlgValidasiSBAR.countBelum(norw, "ralan");
        int nSoap = rekammedis.DlgValidasiSOAP.countBelum(norw, "ralan");
        if (nSbar > 0) {
            lblNotifSBAR.setText("⚠  " + nSbar + " SBAR belum divalidasi — klik untuk validasi");
            lblNotifSBAR.setVisible(true);
        } else {
            lblNotifSBAR.setVisible(false);
        }
        if (nSoap > 0) {
            lblNotifSOAP.setText("⚠  " + nSoap + " SOAP belum divalidasi — klik untuk validasi");
            lblNotifSOAP.setVisible(true);
        } else {
            lblNotifSOAP.setVisible(false);
        }
        susunLayoutSoapie3Kolom();
    ...
    private void pasangRendererListSOAP() {
        final WarnaTable renderer = new WarnaTable() {
            @Override
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                Component comp = super.getTableCellRendererComponent(table, value, isSelected, hasFocus, row, column);
                int modelRow = table.convertRowIndexToModel(row);
                String nama = ambilNilaiTabel(table, modelRow, 24);
                String profesi = ambilNilaiTabel(table, modelRow, 25);
                boolean dokter = adalahSoapDokter(profesi, nama);

                if (isSelected) {
                    comp.setBackground(dokter ? new Color(166, 210, 255) : new Color(180, 228, 188));
                    comp.setForeground(Color.BLACK);
                } else {
                    comp.setBackground(dokter ? new Color(234, 244, 255) : new Color(236, 248, 238));
                    comp.setForeground(Color.BLACK);
                }
                return comp;
            }
        ...
        tbPemeriksaan.setDefaultRenderer(Object.class, renderer);
        tbPemeriksaan.setDefaultRenderer(String.class, renderer);
        tbPemeriksaan.putClientProperty("colorSoapRows", Boolean.TRUE);
        tbPemeriksaan.putClientProperty("dokterNameColumn", Integer.valueOf(24));
        tbPemeriksaan.putClientProperty("dokterProfesiColumn", Integer.valueOf(25));
        for (int col = 0; col < tbPemeriksaan.getColumnCount(); col++) {
            tbPemeriksaan.getColumnModel().getColumn(col).setCellRenderer(renderer);
        }
    ...
    private boolean adalahSoapDokter(String profesi, String nama) {
        String profesiLower = profesi == null ? "" : profesi.trim().toLowerCase();
        String namaLower = nama == null ? "" : nama.trim().toLowerCase();
        return profesiLower.contains("dokter") ||
                namaLower.startsWith("dr.") ||
                namaLower.startsWith("dr ") ||
                namaLower.startsWith("drg.") ||
                namaLower.startsWith("drg ");
    }

 // === PASTEKAN SoapCellRenderer DI SINI ===
    class SoapCellRenderer extends JTextArea implements TableCellRenderer {

        public SoapCellRenderer() {
            setLineWrap(true);
...
private void styleHeader(JTable table) {
    ...
    header.setBackground(new Color(0, 157, 230));   // biru
    header.setForeground(Color.WHITE);              // teks putih
    header.setFont(new Font("Tahoma", Font.BOLD, 12));
    header.setReorderingAllowed(false);
}

private void isiSOAPDariTabel(JTable tb) {
    ...
    String ttv       = nvl(tb.getValueAt(row, 3)); // kolom TTV
    String s         = nvl(tb.getValueAt(row, 4)); // S
    String o         = nvl(tb.getValueAt(row, 5)); // O (Objek)
    String a         = nvl(tb.getValueAt(row, 6)); // A (Asesmen)
    String p         = nvl(tb.getValueAt(row, 7)); // P (Plan)
    String instruksi = nvl(tb.getValueAt(row, 8)); // Instruksi
    String evaluasi  = nvl(tb.getValueAt(row, 9)); // Evaluasi

    // isi field SOAP di kiri (ganti nama komponen sesuai punyamu)
    TKeluhan.setText(s);
    TPemeriksaan.setText(o);
    TPenilaian.setText(a);
    TindakLanjut.setText(p);
    TInstruksi.setText(instruksi);
    TEvaluasi.setText(evaluasi);

    // isi TTV (Suhu, TD, N, RR, dst)
    isiTTVDariString(ttv);
}

// helper nvl versi Object biar gak NPE
private String nvl(Object val) {
    if (val == null) return "";
    return val.toString();
}

private void terapkanObjectTemplateKeSoap(String objectText) {
    HasilParsingObjectTemplate hasil = parseObjectTemplate(objectText);
    TPemeriksaan.setText(hasil.objectText);

    if (hasil.hasTtv()) {
        kosongkanFieldTTV();

        if (!hasil.tensi.equals("")) {
            TTensi.setText(hasil.tensi);
        }
        if (!hasil.nadi.equals("")) {
            TNadi.setText(hasil.nadi);
        }
        if (!hasil.respirasi.equals("")) {
            TRespirasi.setText(hasil.respirasi);
        }
        if (!hasil.suhu.equals("")) {
            TSuhu.setText(hasil.suhu);
        }
        if (!hasil.berat.equals("")) {
            TBerat.setText(hasil.berat);
        }
        if (!hasil.tinggi.equals("")) {
            TTinggi.setText(hasil.tinggi);
        }
        if (!hasil.spo2.equals("")) {
            SpO2.setText(hasil.spo2);
        }
        if (!hasil.gcs.equals("")) {
            TGCS.setText(hasil.gcs);
        }
        if (!hasil.kesadaran.equals("")) {
            pilihKesadaranTemplate(hasil.kesadaran);
        }
    }
}

private HasilParsingObjectTemplate parseObjectTemplate(String objectText) {
    HasilParsingObjectTemplate hasil = new HasilParsingObjectTemplate();
    String sisa = nvl(objectText).trim();
    if (sisa.equals("")) {
        hasil.objectText = "";
        return hasil;
    }

    PolaEkstraksi[] pola = new PolaEkstraksi[]{
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:td|tensi|tekanan\\s*darah)\\s*[:=]?\\s*([0-9]{2,3}\\s*/\\s*[0-9]{2,3}(?:\\s*mmhg)?)"), "tensi"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:nadi)\\s*[:=]?\\s*([0-9]{1,3}(?:[.,][0-9]+)?\\s*(?:x\\s*/?\\s*(?:menit|mnt|m)|bpm)?)"), "nadi"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:n)\\s*[:=]?\\s*([0-9]{1,3}(?:[.,][0-9]+)?\\s*(?:x\\s*/?\\s*(?:menit|mnt|m))?)"), "nadi"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:rr|respirasi|pernafasan|pernapasan|nafas|napas|r)\\s*[:=]?\\s*([0-9]{1,3}(?:[.,][0-9]+)?\\s*(?:x\\s*/?\\s*(?:menit|mnt|m))?)"), "respirasi"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:suhu|temp(?:erature)?|t)\\s*[:=]?\\s*([0-9]{1,2}(?:[.,][0-9]+)?\\s*(?:c|°c)?)"), "suhu"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:bb|berat\\s*badan|berat)\\s*[:=]?\\s*([0-9]{1,3}(?:[.,][0-9]+)?\\s*(?:kg|gram|gr)?)"), "berat"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:tb|tinggi\\s*badan|tinggi)\\s*[:=]?\\s*([0-9]{2,3}(?:[.,][0-9]+)?\\s*(?:cm|m)?)"), "tinggi"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])(?:spo2|sp02|sao2)\\s*[:=]?\\s*([0-9]{1,3}\\s*%)"), "spo2"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])gcs\\s*[:=]?\\s*([A-Za-z0-9./+\\- ]{1,20})"), "gcs"),
        new PolaEkstraksi(Pattern.compile("(?i)(?:^|[\\s,;])kesadaran\\s*[:=]?\\s*([A-Za-z ]{3,30})"), "kesadaran")
    };

    for (PolaEkstraksi item : pola) {
        HasilEkstraksi ekstraksi = ekstrakNilai(sisa, item.pattern);
        if (!ekstraksi.value.equals("")) {
            setNilaiHasilParsing(hasil, item.target, ekstraksi.value);
            sisa = ekstraksi.remaining;
        }
    }

    if (hasil.kesadaran.equals("")) {
        if (sisa.toLowerCase().contains("compos mentis") || sisa.toLowerCase().contains("composmentis")) {
            hasil.kesadaran = "Compos Mentis";
            hasil.foundTtv = true;
        }
    }

    sisa = sisa.replaceAll("(?i)\\bpemeriksaan\\s*objek\\s*:?\\s*", " ");
    sisa = sisa.replaceAll("(?i)\\bku\\s*:?\\s*baik\\b", " ");
    sisa = sisa.replaceAll("\\s+", " ").trim();
    sisa = sisa.replaceAll("^[,;:.\\-\\s]+", "").replaceAll("[,;:.\\-\\s]+$", "").trim();

    hasil.objectText = sisa.equals("") ? nvl(objectText).trim() : sisa;
    return hasil;
}

private HasilEkstraksi ekstrakNilai(String sumber, Pattern pattern) {
    Matcher matcher = pattern.matcher(sumber);
    if (matcher.find()) {
        String fullMatch = matcher.group(0);
        String value = matcher.group(1) == null ? "" : matcher.group(1).trim();
        String remaining = sumber.replaceFirst(Pattern.quote(fullMatch), " ");
        remaining = remaining.replaceAll("\\s+", " ").trim();
        return new HasilEkstraksi(value, remaining);
    }
    return new HasilEkstraksi("", sumber);
}

private void setNilaiHasilParsing(HasilParsingObjectTemplate hasil, String target, String value) {
    String normalized = bersihkanNilaiTTV(target, value);
    if (normalized.equals("")) {
        return;
    }

    if (target.equals("tensi")) {
        hasil.tensi = normalized;
    } else if (target.equals("nadi")) {
        hasil.nadi = normalized;
    } else if (target.equals("respirasi")) {
        hasil.respirasi = normalized;
    } else if (target.equals("suhu")) {
        hasil.suhu = normalized;
    } else if (target.equals("berat")) {
        hasil.berat = normalized;
    } else if (target.equals("tinggi")) {
        hasil.tinggi = normalized;
    } else if (target.equals("spo2")) {
        hasil.spo2 = normalized;
    } else if (target.equals("gcs")) {
        hasil.gcs = normalized;
    } else if (target.equals("kesadaran")) {
        hasil.kesadaran = normalized;
    }
    hasil.foundTtv = true;
}

private String bersihkanNilaiTTV(String target, String value) {
    String hasil = nvl(value).trim();
    if (hasil.equals("")) {
        return "";
    }

    if (target.equals("tensi")) {
        hasil = hasil.replaceAll("(?i)\\s*mmhg\\s*", "");
    } else if (target.equals("nadi") || target.equals("respirasi")) {
        hasil = hasil.replaceAll("(?i)\\s*(x\\s*/?\\s*(menit|mnt|m)|bpm)\\s*", "");
    } else if (target.equals("suhu")) {
        hasil = hasil.replaceAll("(?i)\\s*(c|°c|ºc)\\s*", "");
    } else if (target.equals("berat")) {
        hasil = hasil.replaceAll("(?i)\\s*(kg|gram|gr)\\s*", "");
    } else if (target.equals("tinggi")) {
        hasil = hasil.replaceAll("(?i)\\s*(cm|m)\\s*", "");
    } else if (target.equals("spo2")) {
        hasil = hasil.replace("%", "").trim();
    }

    return hasil.trim();
}

private void kosongkanFieldTTV() {
    TSuhu.setText("");
    TTensi.setText("");
    TBerat.setText("");
    TTinggi.setText("");
...
private void pilihKesadaranTemplate(String kesadaran) {
    for (int idx = 0; idx < cmbKesadaran.getItemCount(); idx++) {
        Object item = cmbKesadaran.getItemAt(idx);
        if (item != null && item.toString().trim().equalsIgnoreCase(kesadaran.trim())) {
            cmbKesadaran.setSelectedIndex(idx);
            return;
        }
    }
}

private void isiTTVDariString(String ttv) {
    if (ttv == null) ttv = "";
    String[] parts = ttv.split(",");

    // kosongkan dulu biar gak ketukar
    TSuhu.setText("");
    TTensi.setText("");
    TBerat.setText("");
    TTinggi.setText("");
    ...
    SpO2.setText("");
    TGCS.setText("");
    // cmbKesadaran.setSelectedIndex(0); // kalau mau reset

    for (String part : parts) {
        String p = part.trim();

        if (p.startsWith("TD ")) {
            TTensi.setText(bersihkanNilaiTTV("tensi", p.substring(3).trim()));
        } else if (p.startsWith("N ")) {
            TNadi.setText(bersihkanNilaiTTV("nadi", p.substring(2).trim()));
        } else if (p.startsWith("RR ")) {
            TRespirasi.setText(bersihkanNilaiTTV("respirasi", p.substring(3).trim()));
        } else if (p.startsWith("Suhu ")) {
            TSuhu.setText(bersihkanNilaiTTV("suhu", p.substring(5).trim()));
        } else if (p.startsWith("BB ")) {
            TBerat.setText(bersihkanNilaiTTV("berat", p.substring(3).trim()));
        } else if (p.startsWith("TB ")) {
            TTinggi.setText(bersihkanNilaiTTV("tinggi", p.substring(3).trim()));
        } else if (p.startsWith("SpO2 ")) {
            SpO2.setText(bersihkanNilaiTTV("spo2", p.substring(5).trim()));
        } else if (p.startsWith("GCS ")) {
            TGCS.setText(p.substring(4).trim());
        } else if (p.startsWith("Kesadaran ")) {
            String kesadaran = p.substring(10).trim();
            // pilih di combo kesadaran kalau ada
            // contoh:
...
private static class PolaEkstraksi {
    private final Pattern pattern;
    private final String target;

    private PolaEkstraksi(Pattern pattern, String target) {
        this.pattern = pattern;
        this.target = target;
    }
}

private static class HasilEkstraksi {
    private final String value;
    private final String remaining;

    private HasilEkstraksi(String value, String remaining) {
        this.value = value;
        this.remaining = remaining;
    }
}

private static class HasilParsingObjectTemplate {
    ...
    private boolean foundTtv = false;

    private boolean hasTtv() {
        return foundTtv;
    }
}
